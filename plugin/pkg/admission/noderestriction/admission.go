...
var (
	...
	nodeResourceCapacityResource = resource.Resource("noderesourcecapacities")
)
...
func (p *Plugin) Admit(ctx context.Context, a admission.Attributes, o admission.ObjectInterfaces) error {
	...
	case nodeResourceCapacityResource:
		return p.admitNodeResourceCapacity(nodeName, a)
...
func (p *Plugin) admitNodeResourceCapacity(nodeName string, a admission.Attributes) error {
	// Shortcut the check if not enabled.
	if !p.dynamicResourceAllocationEnabled {
		return nil
	}
	...
	if a.GetOperation() == admission.Create {
		capacity, ok := a.GetObject().(*resource.NodeResourceCapacity)
		if !ok {
			return admission.NewForbidden(a, fmt.Errorf("unexpected type %T", a.GetObject()))
		}

		if capacity.NodeName != nodeName {
			return admission.NewForbidden(a, errors.New("can only create NodeResourceCapacity with the same NodeName as the requesting node"))
		}
	}
