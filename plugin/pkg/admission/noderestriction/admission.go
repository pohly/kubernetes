...
func (p *Plugin) admitPVCStatus(nodeName string, a admission.Attributes) error {
	...
	case admission.Update:
		...
		if !apiequality.Semantic.DeepEqual(oldPVC, newPVC) {
			return admission.NewForbidden(a, fmt.Errorf("node %q is not allowed to update fields other than status.slice and status.conditions: %v", nodeName, cmp.Diff(oldPVC, newPVC)))
		}
