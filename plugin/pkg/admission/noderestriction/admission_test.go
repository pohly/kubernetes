...
func TestAdmitNodeResourceCapacity(t *testing.T) {
	apiResource := resourceapi.SchemeGroupVersion.WithResource("noderesourcecapacities")
	nodename := "mynode"
	mynode := &user.DefaultInfo{Name: "system:node:" + nodename, Groups: []string{"system:nodes"}}
	err := "can only create NodeResourceCapacity with the same NodeName as the requesting node"

	capacityNode := &resourceapi.NodeResourceCapacity{
		ObjectMeta: metav1.ObjectMeta{
			Name: "something",
		},
		NodeName: nodename,
	}
	capacityOtherNode := &resourceapi.NodeResourceCapacity{
		ObjectMeta: metav1.ObjectMeta{
			Name: "something",
		},
