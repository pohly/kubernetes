...
//  5. If a request is for a noderesourcecapacity, then authorize access if there is an
//     edge from the existing capacity object to the node, which is the case if the
...
var (
	...
	nodeResourceCapacity  = resourceapi.Resource("noderesourcecapacities")
...
func (r *NodeAuthorizer) Authorize(ctx context.Context, attrs authorizer.Attributes) (authorizer.Decision, string, error) {
	...
	if attrs.IsResourceRequest() {
		...
		case nodeResourceCapacity:
			return r.authorizeNodeResourceCapacity(nodeName, attrs)
		}
...
// authorizeNodeResourceCapacity authorizes node requests to NodeResourceCapacity resource.k8s.io/noderesourcecapacities
func (r *NodeAuthorizer) authorizeNodeResourceCapacity(nodeName string, attrs authorizer.Attributes) (authorizer.Decision, string, error) {
	if len(attrs.GetSubresource()) > 0 {
		klog.V(2).Infof("NODE DENY: '%s' %#v", nodeName, attrs)
		return authorizer.DecisionNoOpinion, "cannot authorize NodeResourceCapacity subresources", nil
	}
	...
	default:
		klog.V(2).Infof("NODE DENY: '%s' %#v", nodeName, attrs)
		return authorizer.DecisionNoOpinion, "can only get, create, update, patch, or delete a NodeResourceCapacity", nil
	}

	// The request must come from a node with the same name as the NodeResourceCapacity.NodeName field.
	//
	...
	return r.authorize(nodeName, capacityVertexType, attrs)
}
