...
func New(ctx context.Context, ttl time.Duration) Cache {
	cache := newSchedulerCache(ctx, ttl, cleanAssumedPeriod)
	cache.run(klogr.FromContext(ctx))
	return cache
}
...
func newSchedulerCache(ctx context.Context, ttl, period time.Duration) *schedulerCache {
...
func (cache *schedulerCache) moveNodeInfoToHead(logger klogr.Logger, name string) {
	ni, ok := cache.nodes[name]
	if !ok {
		logger.Error(nil, "No node info with given name found in the cache", "node", klogr.KRef("", name))
		return
	}
...
// linked list.
// We assume cache lock is already acquired.
func (cache *schedulerCache) removeNodeInfoFromList(logger klogr.Logger, name string) {
	ni, ok := cache.nodes[name]
	if !ok {
		logger.Error(nil, "No node info with given name found in the cache", "node", klogr.KRef("", name))
		return
...
func (cache *schedulerCache) UpdateSnapshot(logger klogr.Logger, nodeSnapshot *Snapshot) error {
	cache.mu.Lock()
...
func (cache *schedulerCache) updateNodeInfoSnapshotList(logger klogr.Logger, snapshot *Snapshot, updateAll bool) {
	...
	if updateAll {
		...
		for _, nodeName := range nodesList {
			if nodeInfo := snapshot.nodeInfoMap[nodeName]; nodeInfo != nil {
				...
				logger.Error(nil, "Node exists in nodeTree but not in NodeInfoMap, this should not happen", "node", klogr.KRef("", nodeName))
			}
		}
	} else {
...
func (cache *schedulerCache) AssumePod(logger klogr.Logger, pod *v1.Pod) error {
...
func (cache *schedulerCache) FinishBinding(logger klogr.Logger, pod *v1.Pod) error {
	return cache.finishBinding(logger, pod, time.Now())
}

// finishBinding exists to make tests determinitistic by injecting now as an argument
func (cache *schedulerCache) finishBinding(logger klogr.Logger, pod *v1.Pod, now time.Time) error {
	key, err := framework.GetPodKey(pod)
	if err != nil {
		return err
	}
	...
	logger.V(5).Info("Finished binding for pod, can be expired", "pod", klogr.KObj(pod))
...
func (cache *schedulerCache) ForgetPod(logger klogr.Logger, pod *v1.Pod) error {
	key, err := framework.GetPodKey(pod)
	if err != nil {
		return err
	}
...
func (cache *schedulerCache) addPod(logger klogr.Logger, pod *v1.Pod, assumePod bool) error {
	key, err := framework.GetPodKey(pod)
	if err != nil {
		return err
	}
...
// Assumes that lock is already acquired.
func (cache *schedulerCache) updatePod(logger klogr.Logger, oldPod, newPod *v1.Pod) error {
...
// the cache.
func (cache *schedulerCache) removePod(logger klogr.Logger, pod *v1.Pod) error {
	key, err := framework.GetPodKey(pod)
	if err != nil {
		return err
	}
	...
	n, ok := cache.nodes[pod.Spec.NodeName]
	if !ok {
		logger.Error(nil, "Node not found when trying to remove pod", "node", klogr.KRef("", pod.Spec.NodeName), "pod", klogr.KObj(pod))
	} else {
...
func (cache *schedulerCache) AddPod(logger klogr.Logger, pod *v1.Pod) error {
	key, err := framework.GetPodKey(pod)
	if err != nil {
		return err
	}
	...
	case ok && cache.assumedPods.Has(key):
		if currState.pod.Spec.NodeName != pod.Spec.NodeName {
			...
			logger.Info("Pod was added to a different node than it was assumed", "pod", klogr.KObj(pod), "assumedNode", klogr.KRef("", pod.Spec.NodeName), "currentNode", klogr.KRef("", currState.pod.Spec.NodeName))
...
func (cache *schedulerCache) UpdatePod(logger klogr.Logger, oldPod, newPod *v1.Pod) error {
	...
	if ok && !cache.assumedPods.Has(key) {
		if currState.pod.Spec.NodeName != newPod.Spec.NodeName {
			logger.Error(nil, "Pod updated on a different node than previously added to", "pod", klogr.KObj(oldPod))
			logger.Error(nil, "SchedulerCache is corrupted and can badly affect scheduling decisions")
			os.Exit(1)
		}
...
func (cache *schedulerCache) RemovePod(logger klogr.Logger, pod *v1.Pod) error {
	...
	if currState.pod.Spec.NodeName != pod.Spec.NodeName {
		logger.Error(nil, "Pod was added to a different node than it was assumed", "pod", klogr.KObj(pod), "assumedNode", klogr.KRef("", pod.Spec.NodeName), "currentNode", klogr.KRef("", currState.pod.Spec.NodeName))
...
func (cache *schedulerCache) AddNode(logger klogr.Logger, node *v1.Node) *framework.NodeInfo {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *schedulerCache) UpdateNode(logger klogr.Logger, oldNode, newNode *v1.Node) *framework.NodeInfo {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *schedulerCache) RemoveNode(logger klogr.Logger, node *v1.Node) error {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *schedulerCache) run(logger klogr.Logger) {
	go wait.Until(func() {
...
func (cache *schedulerCache) cleanupAssumedPods(logger klogr.Logger, now time.Time) {
	cache.mu.Lock()
	defer cache.mu.Unlock()
	...
	for key := range cache.assumedPods {
		...
		if !ps.bindingFinished {
			logger.V(5).Info("Could not expire cache for pod as binding is still in progress",
				"pod", klogr.KObj(ps.pod))
			continue
		}
		if now.After(*ps.deadline) {
			logger.Info("Pod expired", "pod", klogr.KObj(ps.pod))
			if err := cache.removePod(logger, ps.pod); err != nil {
				logger.Error(err, "ExpirePod failed", "pod", klogr.KObj(ps.pod))
			}
		}
	}
}
