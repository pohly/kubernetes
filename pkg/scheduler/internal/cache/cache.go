...
func New(ctx context.Context, ttl time.Duration) Cache {
	...
	cache.run(klog.FromContext(ctx))
	return cache
}
...
func (cache *schedulerCache) moveNodeInfoToHead(logger klog.Logger, name string) {
...
// linked list.
// We assume cache lock is already acquired.
func (cache *schedulerCache) removeNodeInfoFromList(logger klog.Logger, name string) {
	ni, ok := cache.nodes[name]
	if !ok {
		logger.Error(nil, "No node info with given name found in the cache", "node", klog.KRef("", name))
		return
...
func (cache *schedulerCache) UpdateSnapshot(logger klog.Logger, nodeSnapshot *Snapshot) error {
	cache.mu.Lock()
...
func (cache *schedulerCache) updateNodeInfoSnapshotList(logger klog.Logger, snapshot *Snapshot, updateAll bool) {
...
func (cache *schedulerCache) AssumePod(logger klog.Logger, pod *v1.Pod) error {
...
func (cache *schedulerCache) FinishBinding(logger klog.Logger, pod *v1.Pod) error {
	return cache.finishBinding(logger, pod, time.Now())
}

// finishBinding exists to make tests determinitistic by injecting now as an argument
func (cache *schedulerCache) finishBinding(logger klog.Logger, pod *v1.Pod, now time.Time) error {
	key, err := framework.GetPodKey(pod)
	if err != nil {
		return err
	}
...
func (cache *schedulerCache) ForgetPod(logger klog.Logger, pod *v1.Pod) error {
	key, err := framework.GetPodKey(pod)
	if err != nil {
		return err
	}
...
func (cache *schedulerCache) addPod(logger klog.Logger, pod *v1.Pod, assumePod bool) error {
	key, err := framework.GetPodKey(pod)
	if err != nil {
		return err
	}
...
// Assumes that lock is already acquired.
func (cache *schedulerCache) updatePod(logger klog.Logger, oldPod, newPod *v1.Pod) error {
...
// the cache.
func (cache *schedulerCache) removePod(logger klog.Logger, pod *v1.Pod) error {
	key, err := framework.GetPodKey(pod)
	if err != nil {
		return err
	}
...
func (cache *schedulerCache) AddPod(logger klog.Logger, pod *v1.Pod) error {
	key, err := framework.GetPodKey(pod)
	if err != nil {
		return err
	}
...
func (cache *schedulerCache) AddNode(logger klog.Logger, node *v1.Node) *framework.NodeInfo {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *schedulerCache) UpdateNode(logger klog.Logger, oldNode, newNode *v1.Node) *framework.NodeInfo {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *schedulerCache) RemoveNode(logger klog.Logger, node *v1.Node) error {
	cache.mu.Lock()
	defer cache.mu.Unlock()
...
func (cache *schedulerCache) run(logger klog.Logger) {
	go wait.Until(func() {
...
func (cache *schedulerCache) cleanupAssumedPods(logger klog.Logger, now time.Time) {
	cache.mu.Lock()
	defer cache.mu.Unlock()
