...
func newNodeTree(logger klogr.Logger, nodes []*v1.Node) *nodeTree {
	nt := &nodeTree{
...
func (nt *nodeTree) addNode(logger klogr.Logger, n *v1.Node) {
	zone := utilnode.GetZoneKey(n)
	if na, ok := nt.tree[zone]; ok {
		for _, nodeName := range na {
			if nodeName == n.Name {
				logger.Info("Node already exists in the NodeTree", "node", klogr.KObj(n))
				return
			}
		}
	...
	logger.V(2).Info("Added node in listed group to NodeTree", "node", klogr.KObj(n), "zone", zone)
	nt.numNodes++
}

// removeNode removes a node from the NodeTree.
func (nt *nodeTree) removeNode(logger klogr.Logger, n *v1.Node) error {
	zone := utilnode.GetZoneKey(n)
	if na, ok := nt.tree[zone]; ok {
		for i, nodeName := range na {
			if nodeName == n.Name {
				...
				logger.V(2).Info("Removed node in listed group from NodeTree", "node", klogr.KObj(n), "zone", zone)
				nt.numNodes--
				return nil
			}
		}
	}
	logger.Error(nil, "Node in listed group was not found", "node", klogr.KObj(n), "zone", zone)
...
func (nt *nodeTree) updateNode(logger klogr.Logger, old, new *v1.Node) {
	var oldZone string
	if old != nil {
