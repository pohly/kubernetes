...
func TestAssumePodScheduled(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			defer cancel()
			cache := newSchedulerCache(ctx, time.Second, time.Second)
...
func assumeAndFinishBinding(logger klogr.Logger, cache *schedulerCache, pod *v1.Pod, assumedTime time.Time) error {
	if err := cache.AssumePod(logger, pod); err != nil {
		return err
	}
...
func TestExpirePod(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			ctx, cancel := context.WithCancel(ctx)
			defer cancel()
			cache := newSchedulerCache(ctx, ttl, time.Second)
...
func TestUpdatePodAndGet(t *testing.T) {
	...
	tests := []struct {
		...
		podToUpdate *v1.Pod
		handler     func(logger klogr.Logger, cache Cache, pod *v1.Pod) error
