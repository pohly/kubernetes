...
func (p *PriorityQueue) activate(logger klog.Logger, pod *v1.Pod) bool {
	...
	if pInfo = p.unschedulableQ.get(pod); pInfo == nil {
		...
		if obj, exists, _ := p.podBackoffQ.Get(newQueuedPodInfoForLookup(pod)); !exists {
			logger.Error(nil, "To-activate pod does not exist in unschedulableQ or backoffQ", "pod", klog.KObj(pod))
			return false
		} else {
...
func (p *PriorityQueue) flushUnschedulableQLeftover(logger klog.Logger) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (npm *nominator) add(logger klog.Logger, pi *framework.PodInfo, nominatingInfo *framework.NominatingInfo) {
	...
	if npm.podLister != nil {
		...
		if _, err := npm.podLister.Pods(pi.Pod.Namespace).Get(pi.Pod.Name); err != nil {
			logger.V(4).Info("Pod doesn't exist in podLister, aborting adding it to the nominator", "pod", klog.KObj(pi.Pod))
			return
		}
	}
