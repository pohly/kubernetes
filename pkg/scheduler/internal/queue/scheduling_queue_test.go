...
func TestHighPriorityFlushUnschedulableQLeftover(t *testing.T) {
	...
	q.flushUnschedulableQLeftover(logger)
...
type operation func(logger klogr.Logger, queue *PriorityQueue, pInfo *framework.QueuedPodInfo)

var (
	add = func(logger klogr.Logger, queue *PriorityQueue, pInfo *framework.QueuedPodInfo) {
		queue.Add(logger, pInfo.Pod)
	}
	addUnschedulablePodBackToUnschedulableQ = func(logger klogr.Logger, queue *PriorityQueue, pInfo *framework.QueuedPodInfo) {
		queue.AddUnschedulableIfNotPresent(logger, pInfo, 0)
	}
	addUnschedulablePodBackToBackoffQ = func(logger klogr.Logger, queue *PriorityQueue, pInfo *framework.QueuedPodInfo) {
		queue.AddUnschedulableIfNotPresent(logger, pInfo, -1)
	}
	addPodActiveQ = func(logger klogr.Logger, queue *PriorityQueue, pInfo *framework.QueuedPodInfo) {
		queue.activeQ.Add(pInfo)
	}
	updatePodActiveQ = func(logger klogr.Logger, queue *PriorityQueue, pInfo *framework.QueuedPodInfo) {
		queue.activeQ.Update(pInfo)
	}
	addPodUnschedulableQ = func(logger klogr.Logger, queue *PriorityQueue, pInfo *framework.QueuedPodInfo) {
		// Update pod condition to unschedulable.
	...
	addPodBackoffQ = func(logger klogr.Logger, queue *PriorityQueue, pInfo *framework.QueuedPodInfo) {
		queue.podBackoffQ.Add(pInfo)
	}
	moveAllToActiveOrBackoffQ = func(logger klogr.Logger, queue *PriorityQueue, _ *framework.QueuedPodInfo) {
		queue.MoveAllToActiveOrBackoffQueue(logger, UnschedulableTimeout, nil)
	}
	flushBackoffQ = func(logger klogr.Logger, queue *PriorityQueue, _ *framework.QueuedPodInfo) {
		queue.clock.(*testingclock.FakeClock).Step(2 * time.Second)
		queue.flushBackoffQCompleted(logger)
	}
	moveClockForward = func(logger klogr.Logger, queue *PriorityQueue, _ *framework.QueuedPodInfo) {
		queue.clock.(*testingclock.FakeClock).Step(2 * time.Second)
	}
)
...
func TestPerPodSchedulingMetrics(t *testing.T) {
	...
	queue.flushUnschedulableQLeftover(logger)
	pInfo, err = queue.Pop()
	if err != nil {
		t.Fatalf("Failed to pop a pod %v", err)
	}
