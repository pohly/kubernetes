...
func (c *Configurator) create(logger klogr.Logger) (*Scheduler, error) {
...
func MakeDefaultErrorFunc(logger klogr.Logger, client clientset.Interface, podLister corelisters.PodLister, podQueue internalqueue.SchedulingQueue, schedulerCache internalcache.Cache) func(*framework.QueuedPodInfo, error) {
	return func(podInfo *framework.QueuedPodInfo, err error) {
		pod := podInfo.Pod
		if err == ErrNoNodesAvailable {
			logger.V(2).Info("Unable to schedule pod; no nodes are registered to the cluster; waiting", "pod", klogr.KObj(pod))
		} else if fitError, ok := err.(*framework.FitError); ok {
			// Inject UnschedulablePlugins to PodInfo, which will be used later for moving Pods between queues efficiently.
			podInfo.UnschedulablePlugins = fitError.Diagnosis.UnschedulablePlugins
			logger.V(2).Info("Unable to schedule pod; no fit; waiting", "pod", klogr.KObj(pod), "err", err)
		} else if apierrors.IsNotFound(err) {
			logger.V(2).Info("Unable to schedule pod, possibly due to node not found; waiting", "pod", klogr.KObj(pod), "err", err)
			...
			logger.Error(err, "Error scheduling pod; retrying", "pod", klogr.KObj(pod))
		}

		// Check if the Pod exists in informer cache.
		cachedPod, err := podLister.Pods(pod.Namespace).Get(pod.Name)
		if err != nil {
			logger.Info("Pod doesn't exist in informer cache", "pod", klogr.KObj(pod), "err", err)
			return
		}
		...
		if len(cachedPod.Spec.NodeName) != 0 {
			logger.Info("Pod has been assigned to node. Abort adding it back to queue.", "pod", klogr.KObj(pod), "node", cachedPod.Spec.NodeName)
			return
		}
