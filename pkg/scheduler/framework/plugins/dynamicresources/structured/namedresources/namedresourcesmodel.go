...
import (
	...
	namedresourcesinternal "k8s.io/kubernetes/pkg/apis/resource/structured/namedresources"
	"k8s.io/kubernetes/pkg/apis/resource/structured/namedresources/cel"
	namedresourcesconversion "k8s.io/kubernetes/pkg/apis/resource/structured/namedresources/v1alpha1"
)
...
type instanceAllocation struct {
	allocated bool
	instance  *namedresourcesinternal.Instance
}
...
func AddResources(m *Model, resources *namedresourcesapi.Resources) {
	if resources == nil {
		return
	}

	// Evaluating the CEL expressions needs the internal representation of the
	// instance attributes. In practice this is a shallow copy of the instance
	// slice and thus cheap.
	var internalResources namedresourcesinternal.Resources
	if err := namedresourcesconversion.Convert_v1alpha1_Resources_To_namedresources_Resources(resources, &internalResources, nil); err != nil {
		// Should never happend due to validation.
		return
	}
	for i := range internalResources.Instances {
		m.instances = append(m.instances, instanceAllocation{instance: &internalResources.Instances[i]})
	}
}
...
func AddAllocation(m *Model, result *namedresourcesapi.AllocationResult) {
	if result == nil {
		return
	}
...
func NewClaimController(filter *namedresourcesapi.Filter, requests []*namedresourcesapi.Request) (*Controller, error) {
	c := &Controller{}
	if filter != nil {
...
func (c *Controller) Allocate(ctx context.Context, model Model) ([]*namedresourcesapi.AllocationResult, error) {
	indices, err := c.allocate(ctx, model)
	if err != nil {
		return nil, err
	}
	...
	results := make([]*namedresourcesapi.AllocationResult, len(c.requests))
	for i := range c.requests {
		results[i] = &namedresourcesapi.AllocationResult{Name: model.instances[indices[i]].instance.Name}
	}
	return results, nil
}
...
func (c *Controller) allocate(ctx context.Context, model Model) ([]int, error) {
	...
	for _, request := range c.requests {
		for i, instance := range instances {
			...
			//
			// A more throrough search would include backtracking because
