...
var (
	...
	resourceClass = toResourceClass(corev1ac.ResourceClass(className))
	...
	podWithClaimTemplate  = toPod(corev1ac.Pod(podName, namespace).WithUID(podUID).WithSpec(corev1ac.PodSpec().WithResourceClaims(corev1ac.PodResourceClaim().WithName(resourceName).WithSource(corev1ac.ClaimSource().WithTemplate(corev1ac.ResourceClaimTemplate())))))
	...
	pendingImmediateClaim           = toResourceClaim(corev1ac.ResourceClaim(claimName, namespace).WithSpec(corev1ac.ResourceClaimSpec().WithAllocationMode(v1.AllocationModeImmediate).WithResourceClassName(className)))
	pendingDelayedClaim             = toResourceClaim(corev1ac.ResourceClaim(claimName, namespace).WithSpec(corev1ac.ResourceClaimSpec().WithAllocationMode(v1.AllocationModeWaitForFirstConsumer).WithResourceClassName(className)))
	pendingDelayedClaim2            = toResourceClaim(corev1ac.ResourceClaim(claimName2, namespace).WithSpec(corev1ac.ResourceClaimSpec().WithAllocationMode(v1.AllocationModeWaitForFirstConsumer).WithResourceClassName(className)))
	deallocatingClaim               = toResourceClaim(corev1ac.ResourceClaim(claimName, namespace).WithStatus(corev1ac.ResourceClaimStatus().WithAllocation(corev1ac.AllocationResult()).WithDeallocationRequested(true)))
	inUseClaim                      = toResourceClaim(corev1ac.ResourceClaim(claimName, namespace).WithStatus(corev1ac.ResourceClaimStatus().WithAllocation(corev1ac.AllocationResult()).WithReservedFor(corev1ac.ResourceClaimUserReference().WithUID(podUID))))
	allocatedClaim                  = toResourceClaim(corev1ac.ResourceClaim(claimName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithUID(podUID).WithController(true)).WithStatus(corev1ac.ResourceClaimStatus().WithAllocation(corev1ac.AllocationResult())))
	allocatedClaimWithWrongTopology = toResourceClaim(corev1ac.ResourceClaim(claimName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithUID(podUID).WithController(true)).WithStatus(corev1ac.ResourceClaimStatus().WithAllocation(corev1ac.AllocationResult().WithAvailableOnNodes(corev1ac.NodeSelector().WithNodeSelectorTerms(corev1ac.NodeSelectorTerm().WithMatchExpressions(corev1ac.NodeSelectorRequirement().WithKey("no-such-label").WithOperator(v1.NodeSelectorOpIn).WithValues("no-such-value")))))))
	allocatedClaimWithGoodTopology  = toResourceClaim(corev1ac.ResourceClaim(claimName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithUID(podUID).WithController(true)).WithStatus(corev1ac.ResourceClaimStatus().WithAllocation(corev1ac.AllocationResult().WithAvailableOnNodes(corev1ac.NodeSelector().WithNodeSelectorTerms(corev1ac.NodeSelectorTerm().WithMatchExpressions(corev1ac.NodeSelectorRequirement().WithKey("nodename").WithOperator(v1.NodeSelectorOpIn).WithValues("worker")))))))
	otherClaim                      = toResourceClaim(corev1ac.ResourceClaim("not-my-claim", namespace))

	scheduling                  = toPodScheduling(corev1ac.PodScheduling(podName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithAPIVersion("v1").WithController(true).WithKind("Pod").WithName(podName).WithUID(podUID)))
	schedulingSelectedPotential = toPodScheduling(corev1ac.PodScheduling(podName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithAPIVersion("v1").WithController(true).WithKind("Pod").WithName(podName).WithUID(podUID)).WithSpec(corev1ac.PodSchedulingSpec().WithSelectedNode(workerNode.Name).WithPotentialNodes(workerNode.Name)))
	schedulingPotential         = toPodScheduling(corev1ac.PodScheduling(podName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithAPIVersion("v1").WithController(true).WithKind("Pod").WithName(podName).WithUID(podUID)).WithSpec(corev1ac.PodSchedulingSpec().WithPotentialNodes(workerNode.Name)))
	schedulingInfo              = toPodScheduling(corev1ac.PodScheduling(podName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithAPIVersion("v1").WithController(true).WithKind("Pod").WithName(podName).WithUID(podUID)).WithSpec(corev1ac.PodSchedulingSpec().WithPotentialNodes(workerNode.Name)).WithStatus(corev1ac.PodSchedulingStatus().WithClaims(corev1ac.ResourceClaimSchedulingStatus().WithPodResourceClaimName(resourceName), corev1ac.ResourceClaimSchedulingStatus().WithPodResourceClaimName(resourceName2))))
	schedulingSelectedInfo      = toPodScheduling(corev1ac.PodScheduling(podName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithAPIVersion("v1").WithController(true).WithKind("Pod").WithName(podName).WithUID(podUID)).WithSpec(corev1ac.PodSchedulingSpec().WithSelectedNode(workerNode.Name).WithPotentialNodes(workerNode.Name)).WithStatus(corev1ac.PodSchedulingStatus().WithClaims(corev1ac.ResourceClaimSchedulingStatus().WithPodResourceClaimName(resourceName), corev1ac.ResourceClaimSchedulingStatus().WithPodResourceClaimName(resourceName2))))
)
...
func TestPreFilter(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		classes     []*v1.ResourceClass
		schedulings []*v1.PodScheduling
		...
		"waiting-for-immediate-allocation": {
			pod:    podWithClaimName,
			claims: []*v1.ResourceClaim{pendingImmediateClaim},
			want: want{
				prefilterOp: result{
					returnValue: nilPreFilterResult,
		...
		"waiting-for-deallocation": {
			pod:    podWithClaimName,
			claims: []*v1.ResourceClaim{deallocatingClaim},
			want: want{
				prefilterOp: result{
					returnValue: nilPreFilterResult,
		...
		"delayed-allocation-missing-class": {
			pod:    podWithClaimName,
			claims: []*v1.ResourceClaim{pendingDelayedClaim},
			want: want{
				prefilterOp: result{
					returnValue: nilPreFilterResult,
				},
				filterOp: perNodeResult{
					workerNode.Name: {
						status: framework.AsStatus(fmt.Errorf(`look up resource class: resourceclass "%s" not found`, className)),
					},
				},
			},
		},
		"delayed-allocation-scheduling-select-immediately": {
			...
			pod:     podWithClaimName,
			claims:  []*v1.ResourceClaim{pendingDelayedClaim},
			classes: []*v1.ResourceClass{resourceClass},
			want: want{
				prefilterOp: result{
					returnValue: nilPreFilterResult,
				},
		...
		"delayed-allocation-scheduling-ask": {
			...
			claims:  []*v1.ResourceClaim{pendingDelayedClaim, pendingDelayedClaim2},
			classes: []*v1.ResourceClass{resourceClass},
			want: want{
				prefilterOp: result{
					returnValue: nilPreFilterResult,
		...
		"delayed-allocation-scheduling-finish": {
			...
			pod:         podWithClaimName,
			claims:      []*v1.ResourceClaim{pendingDelayedClaim},
			schedulings: []*v1.PodScheduling{schedulingInfo},
			classes:     []*v1.ResourceClass{resourceClass},
			want: want{
				prefilterOp: result{
					returnValue: nilPreFilterResult,
				...
				reserveOp: result{
					status: framework.NewStatus(framework.UnschedulableAndUnresolvable, `waiting for resource driver to allocate resource`),
					changes: []any{
						corev1ac.PodScheduling(podName, namespace).WithSpec(corev1ac.PodSchedulingSpec().WithSelectedNode(workerNode.Name)),
					},
				},
			},
		},
		"delayed-allocation-scheduling-completed": {
			...
			want: want{
				...
				reserveOp: result{
					changes: []any{
						corev1ac.ResourceClaim(claimName, namespace).WithStatus(corev1ac.ResourceClaimStatus().WithReservedFor(corev1ac.ResourceClaimUserReference().WithResource("pods").WithName(podName).WithUID(podUID))),
					},
				},
				postbindOp: result{
		...
		"in-use-by-other": {
			...
			claims: []*v1.ResourceClaim{inUseClaim},
			want: want{
				prefilterOp: result{
					returnValue: nilPreFilterResult,
		...
		"wrong-topology": {
			pod:    podWithClaimName,
			claims: []*v1.ResourceClaim{allocatedClaimWithWrongTopology},
			want: want{
				prefilterOp: result{
					returnValue: nilPreFilterResult,
				},
		...
		"good-topology": {
			pod:    podWithClaimName,
			claims: []*v1.ResourceClaim{allocatedClaimWithGoodTopology},
			want: want{
				prefilterOp: result{
					returnValue: nilPreFilterResult,
				},
...
func (tc *testContext) listAll(t *testing.T) (objects []metav1.Object) {
	t.Helper()
	claims, err := tc.client.CoreV1().ResourceClaims("").List(tc.ctx, metav1.ListOptions{})
	require.NoError(t, err, "list claims")
	for _, claim := range claims.Items {
		objects = append(objects, &claim)
	}
	schedulings, err := tc.client.CoreV1().PodSchedulings("").List(tc.ctx, metav1.ListOptions{})
...
func setup(t *testing.T, nodes []*v1.Node, claims []*v1.ResourceClaim, classes []*v1.ResourceClass, schedulings []*v1.PodScheduling) (result *testContext) {
	t.Helper()
	...
	for _, class := range classes {
		_, err := tc.client.CoreV1().ResourceClasses().Create(tc.ctx, class, metav1.CreateOptions{})
		require.NoError(t, err, "create resource class")
	}
	for _, scheduling := range schedulings {
		_, err := tc.client.CoreV1().PodSchedulings(scheduling.Namespace).Create(tc.ctx, scheduling, metav1.CreateOptions{})
