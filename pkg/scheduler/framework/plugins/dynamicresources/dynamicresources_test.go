...
var (
	...
	scheduling                  = toPodScheduling(resourcev1alpha1ac.PodScheduling(podName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithAPIVersion("v1").WithController(true).WithKind("Pod").WithName(podName).WithUID(podUID)))
	...
	schedulingInfo              = toPodScheduling(resourcev1alpha1ac.PodScheduling(podName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithAPIVersion("v1").WithController(true).WithKind("Pod").WithName(podName).WithUID(podUID)).WithSpec(resourcev1alpha1ac.PodSchedulingSpec().WithPotentialNodes(workerNode.Name)).WithStatus(resourcev1alpha1ac.PodSchedulingStatus().WithResourceClaims(resourcev1alpha1ac.ResourceClaimSchedulingStatus().WithPodClaimName(resourceName), resourcev1alpha1ac.ResourceClaimSchedulingStatus().WithPodClaimName(resourceName2))))
	schedulingSelectedInfo      = toPodScheduling(resourcev1alpha1ac.PodScheduling(podName, namespace).WithOwnerReferences(metav1ac.OwnerReference().WithAPIVersion("v1").WithController(true).WithKind("Pod").WithName(podName).WithUID(podUID)).WithSpec(resourcev1alpha1ac.PodSchedulingSpec().WithSelectedNode(workerNode.Name).WithPotentialNodes(workerNode.Name)).WithStatus(resourcev1alpha1ac.PodSchedulingStatus().WithResourceClaims(resourcev1alpha1ac.ResourceClaimSchedulingStatus().WithPodClaimName(resourceName), resourcev1alpha1ac.ResourceClaimSchedulingStatus().WithPodClaimName(resourceName2))))
)
