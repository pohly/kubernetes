...
type stateData struct {
	...
	podScheduling *v1.PodScheduling
...
func (d *stateData) updateClaimStatus(ctx context.Context, index int, claim *v1.ResourceClaim) error {
...
// one from scratch.
func (d *stateData) initializePodScheduling(ctx context.Context, pod *v1.Pod, podSchedulingLister corev1listers.PodSchedulingLister) (*v1.PodScheduling, error) {
	d.mutex.Lock()
...
func (d *stateData) publishPodScheduling(ctx context.Context, podScheduling *v1.PodScheduling) error {
	d.mutex.Lock()
	defer d.mutex.Unlock()
	...
	if loggerV := logger.V(6); loggerV.Enabled() {
		...
		logger.V(5).Info(msg, "podscheduling", podScheduling)
	} else {
...
func (d *stateData) storePodScheduling(podScheduling *v1.PodScheduling) {
	d.mutex.Lock()
	defer d.mutex.Unlock()
...
func statusForClaim(podScheduling *v1.PodScheduling, podClaimName string) *v1.ResourceClaimSchedulingStatus {
...
type dynamicResources struct {
	...
	classLister         corev1listers.ResourceClassLister
...
func New(plArgs runtime.Object, fh framework.Handle) (framework.Plugin, error) {
	return &dynamicResources{
		...
		classLister:         fh.SharedInformerFactory().Core().V1().ResourceClasses().Lister(),
...
func (pl *dynamicResources) podHasClaims(pod *v1.Pod) ([]*v1.ResourceClaim, error) {
	claims := make([]*v1.ResourceClaim, 0, len(pod.Spec.ResourceClaims))
	for _, resource := range pod.Spec.ResourceClaims {
		claimName := resourceclaim.Name(pod, &resource)
		isEphemeral := resource.Source.Template != nil
...
func (pl *dynamicResources) PreScore(ctx context.Context, cs *framework.CycleState, pod *v1.Pod, nodes []*v1.Node) *framework.Status {
	...
	if pending && !haveAllNodes(podScheduling.Spec.PotentialNodes, nodes) {
		...
		numNodes := len(nodes)
		if numNodes > v1.PodSchedulingNodeListMaxSize {
			numNodes = v1.PodSchedulingNodeListMaxSize
		}
		...
		if numNodes == len(nodes) {
			...
			for nodeName := range nodeNames {
				if len(podScheduling.Spec.PotentialNodes) >= v1.PodSchedulingNodeListMaxSize {
					break
				}
...
func (pl *dynamicResources) Unreserve(ctx context.Context, cs *framework.CycleState, pod *v1.Pod, nodeName string) {
	...
	for index, claim := range state.claims {
		if claim.Status.Allocation != nil &&
			...
			claim := claim.DeepCopy()
			reservedFor := make([]v1.ResourceClaimUserReference, 0, len(claim.Status.ReservedFor)-1)
