...
func (pl *dynamicResources) EventsToRegister() []framework.ClusterEvent {
	...
	events := []framework.ClusterEvent{
		...
		{Resource: framework.ResourceClaim, ActionType: framework.Add | framework.Update, IsSchedulable: pl.isSchedulableAfterClaimAddOrUpdate},

		// When a driver has provided additional information, a pod waiting for that information
		// may be schedulable.
		{Resource: framework.PodScheduling, ActionType: framework.Update | framework.Delete, IsSchedulable: pl.isSchedulableAfterPodSchedulingUpdateOrDelete},
...
func (pl *dynamicResources) PreEnqueue(ctx context.Context, pod *v1.Pod) (status *framework.Status) {
	logger := klog.FromContext(ctx)
	defer func() {
		logger.V(3).Info("PreEnqueue", "pod", klog.KObj(pod), "status", status)
	}()
...
// isSchedulableAfterClaimAddOrUpdate is invoked whenever a claim changed. It checks whether
...
func (pl *dynamicResources) isSchedulableAfterClaimAddOrUpdate(oldObj, newObj interface{}, pod *v1.Pod) framework.IsSchedulableResult {
	modifiedClaim, ok := newObj.(*resourcev1alpha1.ResourceClaim)
	if !ok {
		// Shouldn't happen.
	...
	usesClaim := false
	if err := pl.foreachPodResourceClaim(pod, func(_ string, claim *resourcev1alpha1.ResourceClaim) {
		...
		return framework.PodNotSchedulable
	}
	...
	if usesClaim {
		...
		originalClaim, ok := oldObj.(*resourcev1alpha1.ResourceClaim)
		if !ok {
			// Shouldn't happen.
...
// isSchedulableAfterPodSchedulingAddOrUpdate is invoked whenever a
...
func (pl *dynamicResources) isSchedulableAfterPodSchedulingUpdateOrDelete(oldObj, newObj interface{}, pod *v1.Pod) framework.IsSchedulableResult {
	obj := newObj
	if obj == nil {
		// Delete event.
		obj = oldObj
	...
	var podScheduling *resourcev1alpha1.PodScheduling
	switch obj := obj.(type) {
	case cache.DeletedFinalStateUnknown:
		p, ok := obj.Obj.(*resourcev1alpha1.PodScheduling)
		if !ok {
			// Shouldn't happen.
	...
	if podScheduling.Name == pod.Name && podScheduling.Namespace == pod.Namespace {
		// Deleted while pod is still pending?! Let's fix that immediately.
		if oldObj != nil && newObj == nil {
			pl.logger.V(4).Info("PodScheduling for pod got deleted", "pod", klog.KObj(pod))
			return framework.PodImmediatelySchedulable
		}
		...
		if err := pl.foreachPodResourceClaim(pod, func(podResourceName string, claim *resourcev1alpha1.ResourceClaim) {
		...
		if pendingDelayedClaims == 0 {
			pl.logger.V(4).Info("PodScheduling for pod got completed", "pod", klog.KObj(pod))
			return framework.PodImmediatelySchedulable
		}
		...
		// full knowledge about resource availablity (=
		// PodScheduling.*.UnsuitableNodes is complete) but may happen
		// when it doesn't (= PodScheduling.*.UnsuitableNodes had to be
		// truncated).
		if podScheduling.Spec.SelectedNode != "" {
			for _, claimStatus := range podScheduling.Status.ResourceClaims {
				if sliceContains(claimStatus.UnsuitableNodes, podScheduling.Spec.SelectedNode) {
					pl.logger.V(5).Info("PodScheduling has unsuitable selected node", "pod", klog.KObj(pod), "selectedNode", podScheduling.Spec.SelectedNode, "podResourceName", claimStatus.Name)
					return framework.PodImmediatelySchedulable
				}
			}
		}
		...
		pl.logger.V(5).Info("PodScheduling for pod still incomplete", "pod", klog.KObj(pod))
		return framework.PodNotSchedulable
	}

	pl.logger.V(7).Info("PodScheduling for unrelated pod got modified", "pod", klog.KObj(pod), "podScheduling", klog.KObj(podScheduling))
	return framework.PodNotSchedulable
}

func podSchedulingHasClaimInfo(podScheduling *resourcev1alpha1.PodScheduling, podResourceName string) bool {
	for _, claimStatus := range podScheduling.Status.ResourceClaims {
...
func (pl *dynamicResources) foreachPodResourceClaim(pod *v1.Pod, cb func(podResourceName string, claim *resourcev1alpha1.ResourceClaim)) error {
