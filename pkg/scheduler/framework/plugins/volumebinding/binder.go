...
type SchedulerVolumeBinder interface {
	...
	FindPodVolumes(logger klogr.Logger, pod *v1.Pod, boundClaims, claimsToBind []*v1.PersistentVolumeClaim, node *v1.Node) (podVolumes *PodVolumes, reasons ConflictReasons, err error)
	...
	AssumePodVolumes(logger klogr.Logger, assumedPod *v1.Pod, nodeName string, podVolumes *PodVolumes) (allFullyBound bool, err error)

	// RevertAssumedPodVolumes will revert assumed PV and PVC cache.
	RevertAssumedPodVolumes(logger klogr.Logger, podVolumes *PodVolumes)
	...
	BindPodVolumes(logger klogr.Logger, assumedPod *v1.Pod, podVolumes *PodVolumes) error
}
...
// returned.
func (b *volumeBinder) FindPodVolumes(logger klogr.Logger, pod *v1.Pod, boundClaims, claimsToBind []*v1.PersistentVolumeClaim, node *v1.Node) (podVolumes *PodVolumes, reasons ConflictReasons, err error) {
...
func (b *volumeBinder) AssumePodVolumes(logger klogr.Logger, assumedPod *v1.Pod, nodeName string, podVolumes *PodVolumes) (allFullyBound bool, err error) {
	defer func() {
		if err != nil {
	...
	for _, binding := range podVolumes.StaticBindings {
		...
		logger.V(5).Info("AssumePodVolumes: GetBindVolumeToClaim",
			"PV", klogr.KObj(binding.pv),
			"PVC", klogr.KObj(binding.pvc),
			"newPV", klogr.KObj(newPV),
			"dirty", dirty,
		)
		if err != nil {
...
// RevertAssumedPodVolumes will revert assumed PV and PVC cache.
func (b *volumeBinder) RevertAssumedPodVolumes(logger klogr.Logger, podVolumes *PodVolumes) {
...
func (b *volumeBinder) BindPodVolumes(logger klogr.Logger, assumedPod *v1.Pod, podVolumes *PodVolumes) (err error) {
...
func (b *volumeBinder) bindAPIUpdate(logger klogr.Logger, podName string, bindings []*BindingInfo, claimsToProvision []*v1.PersistentVolumeClaim) error {
	if bindings == nil {
	...
	for _, binding = range bindings {
		logger.V(5).Info("Binding", "PV", klogr.KObj(binding.pv), "PVC", klogr.KObj(binding.pvc))
		// TODO: does it hurt if we make an api call and nothing needs to be updated?
		logger.V(2).Info("Claim bound to volume", "PVC", klogr.KObj(binding.pvc), "PV", klogr.KObj(binding.pv))
		newPV, err := b.kubeClient.CoreV1().PersistentVolumes().Update(context.TODO(), binding.pv, metav1.UpdateOptions{})
		if err != nil {
			logger.V(4).Info("Updating PersistentVolume: binding to claim failed", "PV", klogr.KObj(binding.pv), "PVC", klogr.KObj(binding.pvc), "err", err)
			return err
		}
		logger.V(4).Info("Updating PersistentVolume: bound to claim", "PV", klogr.KObj(binding.pv), "PVC", klogr.KObj(binding.pvc))
	...
	for i, claim = range claimsToProvision {
		logger.V(5).Info("Updating claims objects to trigger volume provisioning", "PVC", klogr.KObj(claim))
...
// provisioners
func (b *volumeBinder) checkBindings(logger klogr.Logger, pod *v1.Pod, bindings []*BindingInfo, claimsToProvision []*v1.PersistentVolumeClaim) (bool, error) {
	...
	if err != nil {
		...
		logger.V(4).Info("Could not get a CSINode object for the node", "node", klogr.KObj(node), "err", err)
	}
...
func (b *volumeBinder) isVolumeBound(logger klogr.Logger, pod *v1.Pod, vol *v1.Volume) (bound bool, pvc *v1.PersistentVolumeClaim, err error) {
	pvcName := ""
	isEphemeral := false
	switch {
...
func (b *volumeBinder) isPVCBound(logger klogr.Logger, namespace, pvcName string) (bool, *v1.PersistentVolumeClaim, error) {
	claim := &v1.PersistentVolumeClaim{
	...
	if fullyBound {
		logger.V(5).Info("PVC is fully bound to PV", "PVC", klogr.KObj(pvc), "PV", klogr.KRef("", pvc.Spec.VolumeName))
	} else {
		if pvc.Spec.VolumeName != "" {
			logger.V(5).Info("PVC is not fully bound to PV", "PVC", klogr.KObj(pvc), "PV", klogr.KRef("", pvc.Spec.VolumeName))
		} else {
			logger.V(5).Info("PVC is not bound", "PVC", klogr.KObj(pvc))
		}
	}
...
func (b *volumeBinder) arePodVolumesBound(logger klogr.Logger, pod *v1.Pod) bool {
...
// GetPodVolumes returns a pod's PVCs separated into bound, unbound with delayed binding (including provisioning)
// and unbound with immediate binding (including prebound)
func (b *volumeBinder) GetPodVolumes(logger klogr.Logger, pod *v1.Pod) (boundClaims []*v1.PersistentVolumeClaim, unboundClaimsDelayBinding []*v1.PersistentVolumeClaim, unboundClaimsImmediate []*v1.PersistentVolumeClaim, err error) {
...
func (b *volumeBinder) checkBoundClaims(logger klogr.Logger, claims []*v1.PersistentVolumeClaim, node *v1.Node) (bool, bool, error) {
	csiNode, err := b.csiNodeLister.Get(node.Name)
	if err != nil {
		// TODO: return the error once CSINode is created by default
	...
	for _, pvc := range claims {
		...
		if err != nil {
			logger.V(4).Info("PersistentVolume and node mismatch for pod", "PV", klogr.KRef("", pvName), "err", err)
			return false, true, nil
		}
		logger.V(5).Info("PersistentVolume and node matches for pod", "PV", klogr.KRef("", pvName))
	}
...
func (b *volumeBinder) findMatchingVolumes(logger klogr.Logger, pod *v1.Pod, claimsToBind []*v1.PersistentVolumeClaim, node *v1.Node) (foundMatches bool, bindings []*BindingInfo, unboundClaims []*v1.PersistentVolumeClaim, err error) {
	...
	for _, pvc := range claimsToBind {
		...
		if pv == nil {
			logger.V(4).Info("No matching volumes for pod", "PVC", klogr.KObj(pvc), "node", klogr.KObj(node))
		...
		logger.V(5).Info("Found matching PV for PVC for pod", "PV", klogr.KObj(pv), "PVC", klogr.KObj(pvc), "node", klogr.KObj(node))
	}
...
func (b *volumeBinder) checkVolumeProvisions(logger klogr.Logger, pod *v1.Pod, claimsToProvision []*v1.PersistentVolumeClaim, node *v1.Node) (provisionSatisfied, sufficientStorage bool, dynamicProvisions []*v1.PersistentVolumeClaim, err error) {
	dynamicProvisions = []*v1.PersistentVolumeClaim{}
	...
	for _, claim := range claimsToProvision {
		...
		if provisioner == "" || provisioner == pvutil.NotSupportedProvisioner {
			logger.V(4).Info("Storage class of claim does not support dynamic provisioning", "storageClassName", className, "PVC", klogr.KObj(claim))
			return false, true, nil, nil
		}

		// Check if the node can satisfy the topology requirement in the class
		if !v1helper.MatchTopologySelectorTerms(class.AllowedTopologies, labels.Set(node.Labels)) {
			logger.V(4).Info("Node cannot satisfy provisioning topology requirements of claim", "PVC", klogr.KObj(claim))
			return false, true, nil, nil
		}
...
func (b *volumeBinder) hasEnoughCapacity(logger klogr.Logger, provisioner string, claim *v1.PersistentVolumeClaim, storageClass *storagev1.StorageClass, node *v1.Node) (bool, error) {
	...
	logger.V(4).Info("Node has no accessible CSIStorageCapacity with enough capacity for PVC",
		"PVC", klogr.KObj(claim), "size", sizeInBytes, "storageClass", klogr.KObj(storageClass))
	return false, nil
}
...
func (b *volumeBinder) nodeHasAccess(logger klogr.Logger, node *v1.Node, capacity *storagev1beta1.CSIStorageCapacity) bool {
