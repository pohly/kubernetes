...
func (pl *nonCSILimits) filterVolumes(logger klogr.Logger, pod *v1.Pod, newPod bool, filteredVolumes sets.String) error {
	...
	for i := range volumes {
		...
		if err != nil {
			...
			logger.V(4).Info("Unable to look up PVC info, assuming PVC doesn't match predicate when counting limits", "pod", klogr.KObj(pod), "PVC", klogr.KRef(pod.Namespace, pvcName), "err", err)
			continue
		}
		...
		if pvName == "" {
			...
			if pl.matchProvisioner(pvc) {
				logger.V(4).Info("PVC is not bound, assuming PVC matches predicate when counting limits", "pod", klogr.KObj(pod), "PVC", klogr.KRef(pod.Namespace, pvcName))
		...
		if err != nil {
			...
			if pl.matchProvisioner(pvc) {
				logger.V(4).Info("Unable to look up PV, assuming PV matches predicate when counting limits", "pod", klogr.KObj(pod), "PVC", klogr.KRef(pod.Namespace, pvcName), "PV", klogr.KRef("", pvName), "err", err)
				filteredVolumes.Insert(pvID)
			}
			continue
		}
...
func getMaxVolLimitFromEnv(logger klogr.Logger) int {
...
func getMaxVolumeFunc(logger klogr.Logger, filterName string) func(node *v1.Node) int {
