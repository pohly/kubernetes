...
func (pl *CSILimits) Filter(ctx context.Context, _ *framework.CycleState, pod *v1.Pod, nodeInfo *framework.NodeInfo) *framework.Status {
	...
	if err != nil {
		...
		logger.V(5).Info("Could not get a CSINode object for the node", "node", klogr.KObj(node), "err", err)
	}
...
func (pl *CSILimits) filterAttachableVolumes(
	logger klogr.Logger, pod *v1.Pod, csiNode *storagev1.CSINode, newPod bool, result map[string]string) error {
	for _, vol := range pod.Spec.Volumes {
		...
		if err != nil {
			...
			logger.V(5).Info("Unable to look up PVC info", "pod", klogr.KObj(pod), "PVC", klogr.KRef(pod.Namespace, pvcName))
			continue
		}
...
func (pl *CSILimits) getCSIDriverInfo(logger klogr.Logger, csiNode *storagev1.CSINode, pvc *v1.PersistentVolumeClaim) (string, string) {
	pvName := pvc.Spec.VolumeName

	if pvName == "" {
		logger.V(5).Info("Persistent volume had no name for claim", "PVC", klogr.KObj(pvc))
		return pl.getCSIDriverInfoFromSC(logger, csiNode, pvc)
	}

	pv, err := pl.pvLister.Get(pvName)
	if err != nil {
		logger.V(5).Info("Unable to look up PV info for PVC and PV", "PVC", klogr.KObj(pvc), "PV", klogr.KRef("", pvName))
...
func (pl *CSILimits) getCSIDriverInfoFromSC(logger klogr.Logger, csiNode *storagev1.CSINode, pvc *v1.PersistentVolumeClaim) (string, string) {
	...
	if scName == "" {
		logger.V(5).Info("PVC has no StorageClass", "PVC", klogr.KObj(pvc))
		return "", ""
	}

	storageClass, err := pl.scLister.Get(scName)
	if err != nil {
		logger.V(5).Info("Could not get StorageClass for PVC", "PVC", klogr.KObj(pvc), "err", err)
		return "", ""
	}
