...
func removeFromSlice(logger klogr.Logger, s []*PodInfo, k string) []*PodInfo {
	for i := range s {
		k2, err := GetPodKey(s[i].Pod)
		if err != nil {
			logger.Error(err, "Cannot get pod key", "pod", klogr.KObj(s[i].Pod))
			continue
		}
		if k == k2 {
			// delete the element
...
func (n *NodeInfo) RemovePod(logger klogr.Logger, pod *v1.Pod) error {
	k, err := GetPodKey(pod)
	if err != nil {
		return err
	}
	...
	for i := range n.Pods {
		...
		if err != nil {
			logger.Error(err, "Cannot get pod key", "pod", klogr.KObj(n.Pods[i].Pod))
			continue
		}
		if k == k2 {
			// delete the element
