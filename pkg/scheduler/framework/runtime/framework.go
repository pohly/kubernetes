...
func (f *frameworkImpl) WaitOnPermit(ctx context.Context, pod *v1.Pod) *framework.Status {
	...
	logger := klogr.FromContext(ctx)
	logger.V(4).Info("Pod waiting on permit", "pod", klogr.KObj(pod))
	...
	if !s.IsSuccess() {
		if s.IsUnschedulable() {
			logger.V(4).Info("Pod rejected while waiting on permit", "pod", klogr.KObj(pod), "status", s.Message())
			s.SetFailedPlugin(s.FailedPlugin())
			return s
		}
		err := s.AsError()
		logger.Error(err, "Failed waiting on permit for pod", "pod", klogr.KObj(pod))
