...
type Interface interface {
	...
	PodEligibleToPreemptOthers(logger klog.Logger, pod *v1.Pod, nominatedNodeStatus *framework.Status) bool
...
func (ev *Evaluator) Preempt(ctx context.Context, pod *v1.Pod, m framework.NodeToStatusMap) (*framework.PostFilterResult, *framework.Status) {
	...
	if !ev.PodEligibleToPreemptOthers(logger, pod, m[pod.Status.NominatedNodeName]) {
		logger.V(5).Info("Pod is not eligible for more preemption", "pod", klog.KObj(pod))
...
func (ev *Evaluator) prepareCandidate(logger klog.Logger, c Candidate, pod *v1.Pod, pluginName string) *framework.Status {
	fh := ev.Handler
