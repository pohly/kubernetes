...
type Interface interface {
	...
	PodEligibleToPreemptOthers(logger klogr.Logger, pod *v1.Pod, nominatedNodeStatus *framework.Status) bool
...
func (ev *Evaluator) Preempt(ctx context.Context, pod *v1.Pod, m framework.NodeToStatusMap) (*framework.PostFilterResult, *framework.Status) {
	...
	if err != nil {
		logger.Error(err, "Getting the updated preemptor pod object", "pod", klogr.KRef(podNamespace, podName))
		return nil, framework.AsStatus(err)
	}

	// 1) Ensure the preemptor is eligible to preempt other pods.
	if !ev.PodEligibleToPreemptOthers(logger, pod, m[pod.Status.NominatedNodeName]) {
		logger.V(5).Info("Pod is not eligible for more preemption", "pod", klogr.KObj(pod))
	...
	if status := ev.prepareCandidate(logger, bestCandidate, pod, ev.PluginName); !status.IsSuccess() {
		return nil, status
	}
...
func (ev *Evaluator) findCandidates(ctx context.Context, pod *v1.Pod, m framework.NodeToStatusMap) ([]Candidate, framework.NodeToStatusMap, error) {
	...
	if len(potentialNodes) == 0 {
		logger.V(3).Info("Preemption will not help schedule pod on any node", "pod", klogr.KObj(pod))
		// In this case, we should clean-up any existing nominated node name of the pod.
		if err := util.ClearNominatedNodeName(ev.Handler.ClientSet(), pod); err != nil {
			logger.Error(err, "Cannot clear 'NominatedNodeName' field of pod", "pod", klogr.KObj(pod))
...
func (ev *Evaluator) callExtenders(logger klogr.Logger, pod *v1.Pod, candidates []Candidate) ([]Candidate, *framework.Status) {
		...
		for nodeName, victims := range nodeNameToVictims {
			if victims == nil || len(victims.Pods) == 0 {
				if extender.IsIgnorable() {
					...
					logger.Info("Ignoring node without victims", "node", klogr.KRef("", nodeName))
					continue
				}
...
func (ev *Evaluator) SelectCandidate(logger klogr.Logger, candidates []Candidate) Candidate {
	if len(candidates) == 0 {
		return nil
	}
	if len(candidates) == 1 {
...
func (ev *Evaluator) prepareCandidate(logger klogr.Logger, c Candidate, pod *v1.Pod, pluginName string) *framework.Status {
	fh := ev.Handler
	...
	for _, victim := range c.Victims().Pods {
		...
		if waitingPod := fh.GetWaitingPod(victim.UID); waitingPod != nil {
			...
			logger.Error(err, "Preempting pod", "pod", klogr.KObj(victim), "preemptor", klogr.KObj(pod))
...
func pickOneNodeForPreemption(logger klogr.Logger, nodesToVictims map[string]*extenderv1.Victims) string {
	...
	if latestStartTime == nil {
		...
		logger.Error(errors.New("earliestStartTime is nil for node"), "Should not reach here", "node", klogr.KRef("", minNodes2[0]))
		return minNodes2[0]
	}
	...
	for i := 1; i < lenNodes2; i++ {
		...
		if earliestStartTimeOnNode == nil {
			logger.Error(errors.New("earliestStartTime is nil for node"), "Should not reach here", "node", klogr.KRef("", node))
			continue
		}
...
func getLowerPriorityNominatedPods(logger klogr.Logger, pn framework.PodNominator, pod *v1.Pod, nodeName string) []*v1.Pod {
