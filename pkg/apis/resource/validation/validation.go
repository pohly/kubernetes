...
func validateDeviceRequest(request resource.DeviceRequest, fldPath *field.Path, stored bool) field.ErrorList {
	...
	case resource.DeviceAllocationModeAll:
		if request.Count != 0 {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("count"), request.Count, "a non-zero count is not valid for allocationMode="+string(request.AllocationMode)))
		}
	...
	default:
		allErrs = append(allErrs, field.Invalid(fldPath.Child("allocationMode"), request.Count, fmt.Sprintf("must be one of: %s", strings.Join([]string{string(resource.DeviceAllocationModeAll), string(resource.DeviceAllocationModeExactCount)}, ", "))))
	}
	return allErrs
}
