...
func ValidateClaim(resourceClaim *resource.ResourceClaim) field.ErrorList {
	...
	allErrs = append(allErrs, validateClaimSpec(&resourceClaim.Spec, field.NewPath("spec"))...)
	return allErrs
}

func validateClaimSpec(spec *resource.ResourceClaimSpec, fldPath *field.Path) field.ErrorList {
	...
	allErrs = append(allErrs, validateClaimParameters(spec.ParametersRef, fldPath.Child("parametersRef"))...)
	switch spec.AllocationMode {
	case resource.AllocationModeImmediate, resource.AllocationModeWaitForFirstConsumer:
	default:
...
func validateClaimParameters(ref *resource.ResourceClaimParametersReference, fldPath *field.Path) field.ErrorList {
...
func ValidateClaimStatusUpdate(resourceClaim, oldClaim *resource.ResourceClaim) field.ErrorList {
	...
	allErrs = append(allErrs, validateSliceIsASet(resourceClaim.Status.ReservedFor, resource.ResourceClaimReservedForMaxSize,
		validateClaimUserReference, fldPath.Child("reservedFor"))...)

	// Now check for invariants that must be valid for a ResourceClaim.
	if len(resourceClaim.Status.ReservedFor) > 0 {
		if resourceClaim.Status.Allocation == nil {
			allErrs = append(allErrs, field.Forbidden(fldPath.Child("reservedFor"), "only allocated claims can get reserved"))
		} else {
			if !resourceClaim.Status.Allocation.ShareableResource && len(resourceClaim.Status.ReservedFor) > 1 {
...
func validatePodSchedulingClaims(claimStatuses []resource.ResourceClaimSchedulingStatus, fldPath *field.Path) field.ErrorList {
	...
	for i, claimStatus := range claimStatuses {
		...
		if podClaimNames.Has(claimStatus.PodClaimName) {
			allErrs = append(allErrs, field.Duplicate(fldPath.Index(i), claimStatus.PodClaimName))
		} else {
			podClaimNames.Insert(claimStatus.PodClaimName)
		}
	}
	return allErrs
}
...
func validateClaimTemplateSpec(spec *resource.ResourceClaimTemplateSpec, fldPath *field.Path) field.ErrorList {
