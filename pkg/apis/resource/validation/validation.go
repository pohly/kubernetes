...
func validateResourcePool(pool resource.ResourcePool, fldPath *field.Path) field.ErrorList {
	...
	if pool.Generation < 0 {
		allErrs = append(allErrs, field.Invalid(fldPath.Child("generation"), pool.Generation, "must not be negative"))
	}
	return allErrs
}
...
func validateQualifiedName(name resource.QualifiedName, fldPath *field.Path) field.ErrorList {
	...
	case 2:
		if len(parts[0]) == 0 {
			allErrs = append(allErrs, field.Required(fldPath, "the domain prefix must not be empty"))
		} else {
...
func validateFullyQualifiedName(name resource.FullyQualifiedName, fldPath *field.Path) field.ErrorList {
	...
	if !strings.Contains(string(name), "/") {
		allErrs = append(allErrs, field.Required(fldPath.Child("domain"), "must include a domain prefix"))
	}
	return allErrs
}
