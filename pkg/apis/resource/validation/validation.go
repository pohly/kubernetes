...
func validateDeviceRequest(request resource.DeviceRequest, fldPath *field.Path, stored bool) field.ErrorList {
	...
	case resource.DeviceCountModeAll:
		if request.Count > 0 {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("count"), request.Count, "a non-zero count is not valid for countMode="+string(request.CountMode)))
		}
	case resource.DeviceCountModeExact:
		if request.Count <= 0 {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("count"), request.Count, "must be greater than zero"))
		}
	default:
		allErrs = append(allErrs, field.Invalid(fldPath.Child("countMode"), request.Count, fmt.Sprintf("must be one of: %s", strings.Join([]string{string(resource.DeviceCountModeAll), string(resource.DeviceCountModeExact)}, ", "))))
	}
	return allErrs
}
...
func validateAllocationConfigSource(source resource.AllocationConfigSource, fldPath *field.Path) field.ErrorList {
	...
	default:
		allErrs = append(allErrs, field.Invalid(fldPath, source, fmt.Sprintf("must be one of: %s", strings.Join([]string{string(resource.AllocationConfigSourceClaim), string(resource.AllocationConfigSourceClass)}, ", "))))
	}
	return allErrs
}
...
func validateResourceSliceSpec(spec, oldSpec *resource.ResourceSliceSpec, fldPath *field.Path) field.ErrorList {
	...
	if oldSpec != nil {
		allErrs = append(allErrs, apimachineryvalidation.ValidateImmutableField(spec.Pool, oldSpec.Pool, fldPath.Child("pool"))...)
...
func validateBasicDevice(device resource.BasicDevice, fldPath *field.Path) field.ErrorList {
	var allErrs field.ErrorList
	allErrs = append(allErrs, validateMap(device.Attributes, resource.ResourceSliceMaxAttributesAndCapacitiesPerDevice, validateQualifiedName, validateDeviceAttribute, fldPath.Child("attributes"))...)
	allErrs = append(allErrs, validateMap(device.Capacity, resource.ResourceSliceMaxAttributesAndCapacitiesPerDevice, validateQualifiedName, validateQuantity, fldPath.Child("capacity"))...)
...
func validateDeviceAttribute(attribute resource.DeviceAttribute, fldPath *field.Path) field.ErrorList {
	...
	switch numFields {
	case 0:
		allErrs = append(allErrs, field.Required(fldPath, "exactly one value must be set"))
	case 1:
		// Okay.
	default:
		allErrs = append(allErrs, field.Invalid(fldPath, attribute, "exactly one field must be set, not several"))
	}
	return allErrs
}
...
func validateFullyQualifiedName(name resource.FullyQualifiedName, fldPath *field.Path) field.ErrorList {
	...
	if !strings.Contains(string(name), "/") {
		allErrs = append(allErrs, field.Required(fldPath.Child("domain"), "a fully-qualified name must include a domain prefix"))
	}
	return allErrs
}
