...
func TestValidateResourceSlice(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"bad-multi-pool-name": {
			wantFailures: field.ErrorList{
				field.Invalid(field.NewPath("spec", "pool", "name").Index(0), badName, "a lowercase RFC 1123 subdomain must consist of lower case alphanumeric characters, '-' or '.', and must start and end with an alphanumeric character (e.g. 'example.com', regex used for validation is '[a-z0-9]([-a-z0-9]*[a-z0-9])?(\\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*')"),
				field.Invalid(field.NewPath("spec", "pool", "name").Index(1), badName, "a lowercase RFC 1123 subdomain must consist of lower case alphanumeric characters, '-' or '.', and must start and end with an alphanumeric character (e.g. 'example.com', regex used for validation is '[a-z0-9]([-a-z0-9]*[a-z0-9])?(\\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*')"),
...
func TestValidateResourceSliceUpdate(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"invalid-update-pool": {
			wantFailures: field.ErrorList{field.Invalid(field.NewPath("spec", "pool"), func() any {
				slice := validResourceSlice.DeepCopy()
				slice.Spec.Pool.Name += "-updated"
				return slice.Spec.Pool
			}(), "field is immutable")},
			oldResourceSlice: validResourceSlice,
			update: func(slice *resource.ResourceSlice) *resource.ResourceSlice {
				slice.Spec.Pool.Name += "-updated"
