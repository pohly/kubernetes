...
func TestWorkloadDefaults(t *testing.T) {
	...
	expectedDefaults := map[string]string{
		...
		".Spec.Volumes[0].VolumeSource.Ephemeral.VolumeClaim.VolumeMode":                              `"Filesystem"`,
...
func TestSetDefaultEphemeral(t *testing.T) {
	...
	for _, test := range tests {
		...
		pod1 := obj1.(*v1.Pod)
		if *pod1.Spec.Volumes[0].VolumeSource.Ephemeral.VolumeClaim.VolumeMode != test.expectedVolumeMode {
			t.Errorf("Test %s failed, Expected VolumeMode: %v, but got %v", test.name, test.volumeMode, *pod1.Spec.Volumes[0].VolumeSource.Ephemeral.VolumeClaim.VolumeMode)
		}
	}
}
