...
type PodScheduling struct {
	...
	// PodScheduling object gets deleted when no longer needed.
	//
	// PodScheduling objects where the owning Pod already got deleted no
	// longer need to be updated because they will be removed soon.
	metav1.ObjectMeta `json:"metadata" protobuf:"bytes,1,name=metadata"`
...
type ResourceClaimSchedulingStatus struct {
	// PodClaimName matches the pod.spec.resourceClaims[*].Name field.
	PodClaimName string `json:"podClaimName,omitempty" protobuf:"bytes,1,opt,name=podClaimName"`
...
type ResourceClaimTemplate struct {
	metav1.TypeMeta `json:",inline"`
	// Standard object metadata of the claim template object. The metadata
	// of the claim that gets generated from this template is defined in
	// the spec.
	metav1.ObjectMeta `json:"metadata" protobuf:"bytes,1,name=metadata"`
