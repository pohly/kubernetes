...
var map_DeviceTaint = map[string]string{
	...
	"evictionsPerSecond": "EvictionsPerSecond controls how quickly Pods get evicted if that is the effect of the taint.\n\nEvictions are tracked separately for each taint. Each eviction has a rate limiter which uses the EvictionsPerSecond configured for the taint. Deleting a pod that is evicted by the taint is delayed by that rate limiter.\n\nA pod may be affected by more than one taint. In that case, the rate limit of each taint is applied. The effect is that pods affected by multiple taints get evicted at the smallest rate defined by any of those taints.\n\nThe default is 10 Pods/s.",
}
