...
var map_CSIStorage = map[string]string{
	"":     "CSIStorage represents information about storage provided by a certain CSI driver, like for example current capacity. As in CSIDriver, the name of the CSIStorage objects is the same as the driver name. Unlike CSIDriver, these objects are created dynamically and get updated regularly when there are changes.",
	"info": "The actual information may depend on the storage class and therefore is provided separately for each storage class that uses the driver.",
}
...
var map_CSIStorageByClass = map[string]string{
	"":                 "CSIStorageByClass contains information for one particular storage class of a CSI driver.",
	"storageClassName": "Storage class name, <all>, <ephemeral>.",
	"pools":            "A CSI driver may allocate storage from one or more pools with different attributes. Entries must have unique names inside this list.",
}

func (CSIStorageByClass) SwaggerDoc() map[string]string {
	return map_CSIStorageByClass
}

var map_CSIStorageList = map[string]string{
	"":         "CSIStorageList is a collection of CSIStorage objects.",
	"metadata": "Standard list metadata More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata",
	"items":    "Items is the list of CSIStorage objects.",
}

func (CSIStorageList) SwaggerDoc() map[string]string {
	return map_CSIStorageList
}

var map_CSIStoragePool = map[string]string{
	"":             "CSIStoragePool identifies one particular storage pool and stores the corresponding attributes.",
	"name":         "The name is some user-friendly identifier for this entry.",
	"nodeTopology": "NodeTopology can be used to describe a storage pool that is available only for certain nodes in the cluster. If not set, the pool is consider to be available from all nodes.",
