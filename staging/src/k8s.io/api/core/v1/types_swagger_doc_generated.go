...
var map_AllocationResult = map[string]string{
	...
	"shareableResource": "ShareableResource determines whether the resource supports more than one user at a time.",
}
...
var map_ClaimSource = map[string]string{
	"":                  "ClaimSource references exactly one ResourceClaim, either by name or by embedding a template for a ResourceClaim that will get created by the control plane.\n\nAdditional options might get added in the future, so code using this struct must error out when none of the options that it supports are set.",
...
var map_ResourceClaimSchedulingStatus = map[string]string{
	...
	"unsuitableNodes":      "UnsuitableNodes lists nodes that the claim cannot be allocated for. Nodes listed here will be ignored by the scheduler when selecting a node for a Pod. All other nodes are potential candidates, either because no information is available yet or because allocation might succeed.\n\nA change of the PodSchedulingSpec.PotentialNodes field and/or a failed allocation attempt trigger an update of this field: the driver then checks all nodes listed in PotentialNodes and UnsuitableNodes and updates UnsuitableNodes.\n\nIt must include the prior UnsuitableNodes in this check because the scheduler will not list those again in PotentialNodes but they might still be unsuitable.\n\nThis can change, so the driver also must refresh this information periodically and/or after changing resource allocation for some other ResourceClaim until a node gets selected by the scheduler.\n\nThe size of this field is limited to 256 (= [PodSchedulingNodeListMaxSize]), the same as for PodSchedulingSpec.PotentialNodes.",
}
...
var map_ResourceClaimSpec = map[string]string{
	...
	"allocationMode":    "Allocation can start immediately or when a Pod wants to use the resource. AllocationModeWaitForFirstConsumer is the default.",
}
...
var map_ResourceClaimStatus = map[string]string{
	...
	"deallocationRequested": "DeallocationRequested gets set by the scheduler when it detects the situation where pod scheduling cannot proceed because some claim was allocated for a node that cannot provide some other required resource. It can only be set for claims that are not currently reserved.\n\nThe driver then needs to deallocate this claim and reset the field together with clearing the Allocation field. The scheduler then will try again.\n\nWhile DeallocationRequested is set, no new users may be added to ReservedFor.",
}
...
var map_ResourceClass = map[string]string{
	...
	"parametersRef": "ParametersRef references an arbitrary separate object that may hold parameters that will be used by the driver when allocating a resource that uses this class. The driver will be able to distinguish between parameters stored here and and those stored in ResourceClaimSpec. These parameters here can only be set by cluster administrators.",
