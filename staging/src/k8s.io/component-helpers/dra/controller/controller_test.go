...
func TestController(t *testing.T) {
	...
	withReservedFor := func(claim *corev1.ResourceClaim, pod *corev1.Pod) *corev1.ResourceClaim {
		claim = claim.DeepCopy()
		claim.Status.ReservedFor = append(claim.Status.ReservedFor, corev1.ResourceClaimUserReference{
			Version:  "v1",
			Resource: "pods",
			Name:     pod.Name,
			UID:      pod.UID,
	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			defer informerFactory.WaitForShutdown()
			defer cancel()
