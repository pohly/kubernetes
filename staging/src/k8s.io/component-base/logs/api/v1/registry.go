...
type LogFormatFactory interface {
	...
	Create(options FormatOptions) (log logr.Logger, flush func())
}
...
func RegisterLogFormat(name string, factory LogFormatFactory) {
	logRegistry.register(name, factory)
}
...
func (lfr *logFormatRegistry) register(name string, factory LogFormatFactory) {
	if lfr.frozen {
		panic(fmt.Sprintf("log format registry is frozen, unable to register log format %s", name))
	}
	if _, ok := lfr.registry[name]; ok {
		panic(fmt.Sprintf("log format: %s already exists", name))
	}
