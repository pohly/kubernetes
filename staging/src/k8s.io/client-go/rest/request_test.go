...
func TestURLTemplate(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// non dynamic client
			Request: NewRequestWithClient(uri, "", ClientContentConfig{GroupVersion: schema.GroupVersion{Group: "test"}}, nil, logger).Verb("POST").
		...
		},
		{
			...
			Request: NewRequestWithClient(uri, "", ClientContentConfig{GroupVersion: schema.GroupVersion{Group: "test"}}, nil, logger).Verb("DELETE").
		...
		},
		{
			...
			// /api/$RESOURCEVERSION/namespaces/$NAMESPACE/$RESOURCE/%NAME
			Request: NewRequestWithClient(uriSingleSlash, "", ClientContentConfig{GroupVersion: schema.GroupVersion{Group: "test"}}, nil, logger).Verb("DELETE").
...
func (l levelLogSink) Init(into logr.RuntimeInfo)                     {}
func (l levelLogSink) Info(level int, msg string, kv ...interface{})  {}
func (l levelLogSink) Error(err error, msg string, kv ...interface{}) {}
func (l levelLogSink) WithName(name string) logr.LogSink              { return l }
func (l levelLogSink) WithValues(kv ...interface{}) logr.LogSink      { return l }

var _ logr.LogSink = levelLogSink{}
...
func TestTruncateBody(t *testing.T) {
	...
	for _, test := range tests {
		logger := klogr.New(levelLogSink{level: test.level})
...
func TestThrottledLogger(t *testing.T) {
	...
	logger := logr.New(levelLogSink{level: 0})
	logMessages := 0
	for i := 0; i < 1000; i++ {
