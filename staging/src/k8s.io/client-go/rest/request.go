...
func (r *Request) logger() klog.Logger {
	if r.c != nil {
		return r.c.LoggerOrFallback()
	}
...
func NewRequestWithClient(base *url.URL, versionedAPIPath string, content ClientContentConfig, client *http.Client, logger klog.Logger) *Request {
...
func (b *throttledLogger) attemptToLog(logger klog.Logger) *klog.Logger {
	for _, setting := range b.settings {
		v := logger.V(int(setting.logLevel))
		if v.Enabled() {
...
func (b *throttledLogger) Info(logger klog.Logger, message string, args ...interface{}) {
...
func (r *Request) newStreamWatcher(logger klog.Logger, resp *http.Response) (watch.Interface, error) {
	...
	for {
		...
		default:
				...
				if retry {
					...
					klog.FromContext(ctx).V(4).Info("Could not retry request", "err", err)
				}
...
func (r *Request) DoRaw(ctx context.Context) ([]byte, error) {
	...
	err := r.request(ctx, func(req *http.Request, resp *http.Response) {
		...
		logBody(klog.FromContext(ctx), "Response Body", result.body)
...
func logBody(logger klog.Logger, prefix string, body []byte) {
	v := logger.V(8)
	if v.Enabled() {
...
func (r Result) Error() error {
	...
	if err != nil {
		...
		// kklog.V(5).Infof("body was not decodable (unable to check for Status): %v", err)
		return r.err
	}
	switch t := out.(type) {
	case *metav1.Status:
