	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				in, err := client.ResourceV1alpha3().ResourceSlices().List(context.TODO(), options)
				if err != nil {
					return nil, err
				}
				var out ResourceSliceList
				if err := Convert_v1alpha3_ResourceSliceList_To_api_ResourceSliceList(in, &out, nil); err != nil {
					return nil, err
				}
				return &out, nil
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				in, err := client.ResourceV1alpha3().ResourceSlices().Watch(context.TODO(), options)
				if err != nil {
					return nil, err
				}
...
func (w *watchResourceSlice) run() {
	...
	for {
		...
		case *resourceapi.ResourceSlice:
			...
			if err := Convert_v1alpha3_ResourceSlice_To_api_ResourceSlice(in, &out, nil); err != nil {
