	...
	for claimIndex, claim := range alloc.claimsToAllocate {
		...
		for i, constraint := range claim.Spec.Devices.Constraints {
			...
			case constraint.MatchAttribute != nil:
				matchAttribute := api.FullyQualifiedName(*constraint.MatchAttribute)
				logger := alloc.logger
...
type constraint interface {
	...
	add(requestName string, device *api.BasicDevice, deviceID DeviceID) bool

	// For every successful add there is exactly one matching removed call
	// with the exact same parameters.
	remove(requestName string, device *api.BasicDevice, deviceID DeviceID)
}
...
type matchAttributeConstraint struct {
	...
	attributeName api.FullyQualifiedName

	attribute  *api.DeviceAttribute
	numDevices int
}

func (m *matchAttributeConstraint) add(requestName string, device *api.BasicDevice, deviceID DeviceID) bool {
...
func (m *matchAttributeConstraint) remove(requestName string, device *api.BasicDevice, deviceID DeviceID) {
	if m.requestNames.Len() > 0 && !m.requestNames.Has(requestName) {
		// Device not affected by constraint.
		return
	}
...
func (alloc *allocator) isSelectable(r requestIndices, slice *api.ResourceSlice, deviceIndex int) (bool, error) {
...
func (alloc *allocator) selectorsMatch(r requestIndices, device *api.BasicDevice, deviceID DeviceID, class *resourceapi.DeviceClass, selectors []resourceapi.DeviceSelector) (bool, error) {
	for i, selector := range selectors {
		...
		api.Convert_api_BasicDevice_To_v1alpha3_BasicDevice(device, &d, nil)
...
func (alloc *allocator) allocateDevice(r deviceIndices, device *api.BasicDevice, deviceID DeviceID, must bool) (bool, func(), error) {
	claim := alloc.claimsToAllocate[r.claimIndex]
...
func (alloc *allocator) findSlice(deviceAllocation resourceapi.DeviceRequestAllocationResult) *api.ResourceSlice {
	driverName := api.MakeUniqueString(deviceAllocation.Driver)
	poolName := api.MakeUniqueString(deviceAllocation.Pool)
	deviceName := api.MakeUniqueString(deviceAllocation.Device)
	for _, pool := range alloc.pools {
