...
// attribute is used to generate device configuration parameters in a form of JSON {atribute: attributeValue}.
...
// attribute is used to generate parameters in a form of JSON {atribute: attributeValue}.
...
func TestAllocator(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"all-devices": {
			claimsToAllocate: objects(claimWithRequests(claim0, nil, resourceapi.DeviceRequest{
				Name:            req0,
				CountMode:       resourceapi.DeviceCountModeAll,
				Count:           1,
				DeviceClassName: classA,
			})),
			classes: objects(class(classA, driverA)),
