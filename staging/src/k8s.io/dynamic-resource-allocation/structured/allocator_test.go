...
// nodeSelector creates a node selector with one requirement, either for labels or fields.
func nodeSelector(match nodeSelectorMatch, key string, operator v1.NodeSelectorOperator, values ...string) *v1.NodeSelector {
	requirements := []v1.NodeSelectorRequirement{{
		Key:      key,
		Operator: operator,
		Values:   values,
	}}
	selector := &v1.NodeSelector{NodeSelectorTerms: []v1.NodeSelectorTerm{{}}}
	if match == nodeSelectorMatchFields {
		selector.NodeSelectorTerms[0].MatchFields = requirements
	} else {
		selector.NodeSelectorTerms[0].MatchExpressions = requirements
	}
	return selector
}

type nodeSelectorMatch string

const (
	nodeSelectorMatchFields = nodeSelectorMatch("fields")
	nodeSelectorMatchLabels = nodeSelectorMatch("labels")
)

// localNodeSelector returns a node selector for a specific node.
func localNodeSelector(nodeName string) *v1.NodeSelector {
	return nodeSelector(nodeSelectorMatchFields, "metadata.name", v1.NodeSelectorOpIn, nodeName)
}
...
func allocationResult(selector *v1.NodeSelector, results ...resourceapi.DeviceRequestAllocationResult) types.GomegaMatcher {
	return gstruct.PointTo(gstruct.MatchFields(0, gstruct.Fields{
		...
		"NodeSelector": gomega.Equal(selector),
	...
	testcases := map[string]struct {
		...
		"network-attached-device-with-class.SuitableNodes": {
			...
			slices: objects(slice(slice1, "", pool1, driverA, 1, 1,
				nodeSelector(nodeSelectorMatchLabels, "region", v1.NodeSelectorOpIn, region1),
				[]resourceapi.Device{
					device(device1, nil, nil),
		...
		"network-attached-device-without-class.SuitableNodes": {
			claimsToAllocate: objects(claim(claim0, req0, classA)),
			classes:          objects(class(classA, driverA)),
			// TODO (?): make this really network-attached
			slices: objects(sliceWithOneDevice(slice1, node1, pool1, driverA, 1, 1)),
			node:   node(node1, region1),
