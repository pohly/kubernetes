...
type Driver interface {
	...
	GetClassParameters(ctx context.Context, class *v1.ResourceClass) (interface{}, error)
	...
	//
	// The caller will wrap the error to include the parameter reference.
	GetClaimParameters(ctx context.Context, claim *v1.ResourceClaim, class *v1.ResourceClass, classParameters interface{}) (interface{}, error)
	...
	// must be idempotent.
	Allocate(ctx context.Context, claim *v1.ResourceClaim, claimParameters interface{}, class *v1.ResourceClass, classParameters interface{}, selectedNode string) (*v1.AllocationResult, error)
	...
	Deallocate(ctx context.Context, claim *v1.ResourceClaim) error
...
func (ctrl *controller) getCachedClaim(ctx context.Context, key string) (*v1.ResourceClaim, error) {
	...
	claim, ok := claimObj.(*v1.ResourceClaim)
	if !ok {
		return nil, fmt.Errorf("internal error: got %T instead of *v1.ResourceClaim from claim cache", claimObj)
	}
	return claim, nil
}
...
// and does it.
func (ctrl *controller) syncClaim(ctx context.Context, claim *v1.ResourceClaim) error {
	var err error
...
func (ctrl *controller) getParameters(ctx context.Context, claim *v1.ResourceClaim, class *v1.ResourceClass) (claimParameters, classParameters interface{}, err error) {
...
func (ctrl *controller) allocateClaim(ctx context.Context,
	claim *v1.ResourceClaim, claimParameters interface{},
	class *v1.ResourceClass, classParameters interface{},
	selectedNode string,
	selectedUser *v1.ResourceClaimUserReference) error {
	logger := klog.FromContext(ctx)
...
func (ctrl *controller) checkPodClaim(ctx context.Context, pod *v1.Pod, podClaim v1.PodResourceClaim) (*ClaimAllocation, error) {
	...
	if podClaim.Source.Template != nil {
		if err := resourceclaim.IsForPod(pod, claim); err != nil {
			return nil, err
		}
	}
	if claim.Spec.AllocationMode != v1.AllocationModeWaitForFirstConsumer {
...
// and does it.
func (ctrl *controller) syncPodScheduling(ctx context.Context, podScheduling *v1.PodScheduling) error {
	logger := klog.FromContext(ctx)
	...
	if selectedNode != "" {
		...
		if unsuitable {
			...
			selectedUser := &v1.ResourceClaimUserReference{
				Resource: "pods",
				Name:     pod.Name,
				UID:      pod.UID,
			}
	...
	for _, delayed := range claims {
		...
		if i < 0 {
			...
			podScheduling.Status.Claims = append(podScheduling.Status.Claims,
				v1.ResourceClaimSchedulingStatus{
...
func (ctrl *controller) hasFinalizer(claim *v1.ResourceClaim) bool {
