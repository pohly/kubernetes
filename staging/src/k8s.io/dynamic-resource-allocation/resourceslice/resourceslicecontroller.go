...
// ResourceSlices.
func StartController(ctx context.Context, kubeClient kubernetes.Interface, driverName string, owner Owner, resources *DriverResources) *Controller {
	if kubeClient == nil {
		return nil
	}
...
func (c *Controller) syncPool(ctx context.Context, poolName string) error {
	...
	if pool, ok := resources.Pools[poolName]; ok {
		...
		slice.Spec.Driver = c.driverName
