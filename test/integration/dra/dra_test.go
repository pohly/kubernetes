...
func (scheduler *schedulerSingleton) start(tCtx ktesting.TContext) {
	...
	schedulerCtx, scheduler.cancel = context.WithCancelCause(schedulerCtx)
	_, scheduler.informerFactory = util.StartScheduler(schedulerCtx, tCtx.Client(), tCtx.RESTConfig(), newDefaultSchedulerComponentConfig(tCtx), nil)
	tCtx.Log("Scheduler started.")
}

func (scheduler *schedulerSingleton) stop() {
	scheduler.mutex.Lock()
	defer scheduler.mutex.Unlock()
