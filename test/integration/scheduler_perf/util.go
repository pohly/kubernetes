	...
	for {
		...
		case <-ticker.C:
			...
			if math.Abs(errorMargin) > 5 {
				...
				if skipped > 0 {
					tc.tb.Errorf("ERROR: Expected throuput collector to sample at regular time intervals. Together, the %d most recent intervals took %s instead of %s, a difference of %0.1f%%.", skipped+1, duration, expectedDuration, errorMargin)
				} else {
					tc.tb.Errorf("ERROR: Expected throuput collector to sample at regular time intervals. The last interval took %s instead of %s, a difference of %0.1f%%.", duration, expectedDuration, errorMargin)
				}
			}
