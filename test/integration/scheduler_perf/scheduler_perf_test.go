...
func runWorkload(ctx context.Context, b *testing.B, tc *testCase, w *workload, cleanup bool) []DataItem {
	...
	for opIndex, op := range unrollWorkloadTemplate(b, tc.WorkloadTemplate, w) {
		...
		case *createPodsOp:
			...
			maybeCreateNamespace(ctx, b, client, namespace, &numPodsScheduledPerNamespace)
...
func maybeCreateNamespace(ctx context.Context, b *testing.B, client clientset.Interface, namespace string, podsPerNamespace *map[string]int) {
