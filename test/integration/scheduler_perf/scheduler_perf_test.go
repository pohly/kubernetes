...
type testCase struct {
	...
	// Labels can be used enable or disable test cases.
	Labels []string
}
...
type workload struct {
	...
	Params params
	// Labels can be used enable or disable workloads.
	Labels []string
}
		...
		b.Run(tc.Name, func(b *testing.B) {
			for _, w := range tc.Workloads {
				b.Run(w.Name, func(b *testing.B) {
					...
					informerFactory, client, dyncClient := setupSchedulerForWorkload(ctx, b, tc.SchedulerConfigPath, tc.FeatureGates)
...
var testSchedulingLabelFilter = flag.String("test-scheduling-label-filter", "integration-test", "comma-separated list of labels which a testcase must have (no prefix or +) or must not have (-)")
...
func TestScheduling(t *testing.T) {
	...
	// All unit test case share the same etcd.
	...
	for _, config := range configs {
		...
		func() {
			...
			informerFactory, client, dynClient := setupSchedulerForWorkload(ctx, t, config.schedulerConfigPath, config.featureGates)
...
func (c schedulerConfig) equals(tc *testCase) bool {
	return c.schedulerConfigPath == tc.SchedulerConfigPath &&
		cmp.Diff(c.featureGates, tc.FeatureGates) == ""
}
...
func setupSchedulerForWorkload(ctx context.Context, tb testing.TB, configPath string, featureGates map[featuregate.Feature]bool) (informers.SharedInformerFactory, clientset.Interface, dynamic.Interface) {
	...
	return mustSetupScheduler(ctx, tb, cfg, featureGates)
}
...
func runWorkload(ctx context.Context, tb testing.TB, tc *testCase, w *workload, informerFactory informers.SharedInformerFactory, client clientset.Interface, dynClient dynamic.Interface, cleanup bool) []DataItem {
	...
	// Those are kept running because they might be needed again.
		...
		defer func() {
			...
			for namespace := range numPodsScheduledPerNamespace {
				...
				if err := client.CoreV1().Namespaces().Delete(ctx, namespace, deleteNow); err != nil {
					tb.Fatalf("Deleting Namespace in numPodsScheduledPerNamespace: %v", err)
				}
			}
	...
	for opIndex, op := range unrollWorkloadTemplate(tb, tc.WorkloadTemplate, w) {
		...
		case *createPodsOp:
			...
			maybeCreateNamespace(ctx, tb, client, namespace, &numPodsScheduledPerNamespace)
			...
			if concreteOp.CollectMetrics {
				...
				collectors = getTestDataCollectors(podInformer, fmt.Sprintf("%s/%s", tb.Name(), namespace), namespace, tc.MetricsCollectorConfig)
			...
			if concreteOp.Sequentially {
				data, err := createPodsSequentially(ctx, tb, namespace, concreteOp, client, podInformer)
				if err != nil {
					tb.Fatalf("op %d: %v", opIndex, err)
				}
...
func maybeCreateNamespace(ctx context.Context, tb testing.TB, client clientset.Interface, namespace string, podsPerNamespace *map[string]int) {
...
func createPodsSequentially(ctx context.Context, tb testing.TB, namespace string, cpo *createPodsOp, clientset clientset.Interface, podInformer coreinformers.PodInformer) ([]DataItem, error) {
	if cpo.Count <= 0 {
		return nil, nil
	}
	...
	for i := 0; i < cpo.Count; i++ {
		...
		tb.Logf("pod %s scheduled", pod.Name)
