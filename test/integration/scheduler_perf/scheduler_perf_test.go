...
func isValidCount(allowParameterization bool, count int, countParam string) bool {
	...
	if !allowParameterization || countParam == "" {
		...
		ok = isValidParameterizable(countParam)
	}
	return ok
}
			...
			for _, w := range tc.Workloads {
				b.Run(w.Name, func(b *testing.B) {
					...
					for _, item := range results {
						...
						destFile, err := dataFilename(fmt.Sprintf("%s_%s_%s.dat", tc.Name, w.Name, runID))
						if err != nil {
...
func runWorkload(ctx context.Context, b *testing.B, tc *testCase, w *workload) []DataItem {
	...
	maybeCreateNamespace := func(namespace string) {
	...
	for opIndex, op := range unrollWorkloadTemplate(b, tc.WorkloadTemplate, w) {
		...
		case *createPodsOp:
			...
			maybeCreateNamespace(namespace)
