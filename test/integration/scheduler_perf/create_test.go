...
func (c *createAny) run(ctx context.Context, tb testing.TB, client clientSet) {
	var obj *unstructured.Unstructured
	if err := getSpecFromFile(&c.TemplatePath, &obj); err != nil {
		tb.Fatalf("%s: parsing failed: %v", c.TemplatePath, err)
	}
	...
	if err != nil {
		tb.Fatalf("%s: extract group+version from object %q: %v", c.TemplatePath, klog.KObj(obj), err)
	}
	...
	create := func() error {
		...
		if c.Namespace != "" {
			...
			_, err = resourceClient.Namespace(c.Namespace).Create(ctx, obj, metav1.CreateOptions{})
		} else {
	...
	for {
		...
		select {
		case <-ctx.Done():
			tb.Fatalf("%s: timed out while creating %q, last error was: %v", c.TemplatePath, klog.KObj(obj), err)
