				...
				b.Run(w.Name, func(b *testing.B) {
					...
					for _, item := range results {
						...
						for _, sample := range item.progress {
							fmt.Fprintf(f, "%.1fs %d %d %d %f %d %d %d %d\n", sample.ts.Sub(item.start).Seconds(), sample.completed, sample.attempts, sample.observedTotal, sample.observedRate, sample.activePods, sample.backoffPods, sample.unschedulable, sample.gatedPods)
						}
