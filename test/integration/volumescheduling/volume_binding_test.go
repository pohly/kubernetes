...
func TestCapacity(t *testing.T) {
	...
	run := func(t *testing.T, test testcaseType) {
		...
		if test.haveCapacity {
			if _, err := config.client.StorageV1alpha1().CSIStorageCapacities().Create(context.TODO(),
...
func deleteTestObjects(client clientset.Interface, ns string, option metav1.DeleteOptions) {
	...
	client.StorageV1alpha1().CSIStorageCapacities().DeleteCollection(context.TODO(), option, metav1.ListOptions{})
}
