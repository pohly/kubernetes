...
type driverDefinition struct {
	...
	StorageClass struct {
		...
		// FromFile is used only when FromName is false.
		// It loads a storage class from the given .yaml or .json
		// file. File names are resolved by the framework.testfiles
		// package, which typically means that they can be absolute or
		// relative to the test suite's --repo-root parameter.
		FromFile string
	}
