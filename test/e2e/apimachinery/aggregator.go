...
func TestSampleAPIServer(ctx context.Context, f *framework.Framework, aggrclient *aggregatorclient.Clientset, image string) {
	...
	gomega.Expect(u.GetAPIVersion()).To(gomega.Equal("wardle.example.com/v1alpha1"))
	...
	if !foundUpdatedStatusCondition {
		ginkgo.Fail(fmt.Sprintf("The updated status condition was not found.\n%s", format.Object(wardle.Status.Conditions, 1)))
	}
	...
	gomega.Expect(updatedApiService.Labels[apiServiceName]).To(gomega.Equal("updated"), "should have the updated label but have %q", updatedApiService.Labels[apiServiceName])
	...
	if !foundPatchedStatusCondition {
		ginkgo.Fail(fmt.Sprintf("The patched status condition was not found.\n%s", format.Object(wardle.Status.Conditions, 1)))
	}
