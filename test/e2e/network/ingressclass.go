...
var _ = common.SIGDescribe("IngressClass API", func() {
	...
	*/
	framework.ConformanceIt(" should support creating IngressClass API operations", func(ctx context.Context) {
		...
		framework.ExpectNoError(err)
		gomega.Expect(patchedIC.Annotations["patched"]).To(gomega.Equal("true"), "patched object should have the applied annotation")
		...
		framework.ExpectNoError(err)
		gomega.Expect(updatedIC.Annotations["updated"]).To(gomega.Equal("true"), "updated object should have the applied annotation")
