...
var _ = common.SIGDescribe("Ingress API", func() {
	...
	*/
	...
	framework.ConformanceIt("should support creating Ingress API operations", func(ctx context.Context) {
		...
		framework.ExpectNoError(err)
		gomega.Expect(patchedIngress.Annotations["patched"]).To(gomega.Equal("true"), "patched object should have the applied annotation")
		...
		framework.ExpectNoError(err)
		gomega.Expect(updatedIngress.Annotations["updated"]).To(gomega.Equal("true"), "updated object should have the applied annotation")
		...
		framework.ExpectNoError(err)
		gomega.Expect(updatedStatus.Status.LoadBalancer).To(gomega.Equal(statusToUpdate.Status.LoadBalancer), fmt.Sprintf("updated object expected to have updated loadbalancer status %#v, got %#v", statusToUpdate.Status.LoadBalancer, updatedStatus.Status.LoadBalancer))
		...
		framework.ExpectNoError(err)
		gomega.Expect(string(createdIngress.UID)).To(gomega.Equal(statusUID), fmt.Sprintf("createdIngress.UID: %v expected to match statusUID: %v ", createdIngress.UID, statusUID))
