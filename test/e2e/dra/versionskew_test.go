...
func init() {
	flag.StringVar(&repoRoot, "repo-root", repoRootDefault(), "Root directory of Kubernetes repository, for finding test files.")
}

// repoRootDefault figures out whether the test is invoked in its directory (as in `go test ./test/e2e/dra`)
// or somewhere else (for example, after compiling a test binary).
func repoRootDefault() string {
	for i := 0; i < 10; i++ {
		path := "." + strings.Repeat("/..", i)
		if _, err := os.Stat(path + "/test/e2e/dra"); err == nil {
			return path
		}
	}
	// Probably not right, but the error message when using it should make it clear.
	return "."
}
...
func TestWithKind(t *testing.T) {
	_, ok := os.LookupEnv("KIND_COMMAND")
	if !ok {
		t.Skip("KIND_COMMAND must be set and kind must be operational.")
	}

	t.Run("upgrade/downgrade works", func(t *testing.T) {
		tCtx := ktesting.Init(t)

		// This is a stop-gap solution to support calling some test/e2e/framework helpers which,
		// against the common convention, make assertions instead of returning an error.
		gomega.RegisterFailHandler(func(failure string, callerSkip ...int) { tCtx.Fatal(failure) })
		defer gomega.RegisterFailHandler(nil)

		// Another stop-gap: redirect logging from code which ignores the context.
		// But this doesn't help with output through framework.Logf, that writes
		// to ginkgo.GinkgoOutput directly because it wants to support GinkgoHelper.
		//
		// TODO: rewrite more common helper code to use ktesting.TContext.
		state := klog.CaptureState()
		defer state.Restore()
		klog.SetLoggerWithOptions(tCtx.Logger(), klog.ContextualLogger(true))

		// Because of these changes to global state, tests cannot run in parallel.
		// Without them they could because multiple clusters may co-exist.
		...
		readyCtx := ktesting.WithRESTConfig(tCtx, restConfig)
		ready := e2enode.CheckReadyForTests(readyCtx, readyCtx.Client(), "" /* no ignored taints */, 1 /* control plane node is not schedulable */, 100 /* large cluster threshold */)
		ktesting.Eventually(readyCtx, func(tCtx ktesting.TContext) bool {
			okay, err := ready(tCtx)
			if err != nil {
				gomega.StopTrying(fmt.Sprintf("permanent failure: %v", err))
				return false
			}
			return okay
		}).WithPolling(5 * time.Second).Should(gomega.BeTrueBecause("cluster nodes should be ready"))
		tCtx = ktesting.End(tCtx)
		...
		// TODO: namespace support in ktesting
		...
		tCtx = WithNamespace(tCtx, namespace.Name)
		...
		gomega.NewWithT(tCtx).Eventually(tCtx, driver.NewGetSlices(tCtx)).WithTimeout(5 * time.Minute).Should(gomega.HaveField("Items", gomega.HaveLen(len(nodes.NodeNames))))
		tCtx = ktesting.End(tCtx)

		// Remove pod prepared by previous Kubernetes.
		tCtx.ExpectNoError(tCtx.Client().ResourceV1beta1().ResourceClaims(namespace.Name).Delete(tCtx, claim.Name, metav1.DeleteOptions{}))
		tCtx.ExpectNoError(tCtx.Client().CoreV1().Pods(namespace.Name).Delete(tCtx, pod.Name, metav1.DeleteOptions{}))
		tCtx.ExpectNoError(e2epod.WaitForPodNotFoundInNamespace(tCtx, tCtx.Client(), pod.Name, namespace.Name, f.Timeouts.PodDelete))
		...
		ktesting.Eventually(tCtx, func(tCtx ktesting.TContext) error {
			return tCtx.Client().ResourceV1beta1().ResourceClaims(namespace.Name).Delete(tCtx, claim.Name, metav1.DeleteOptions{})
		}).Should(gomega.Succeed(), "delete claim after downgrade")
		ktesting.Eventually(tCtx, func(tCtx ktesting.TContext) error {
			return tCtx.Client().CoreV1().Pods(namespace.Name).Delete(tCtx, pod.Name, metav1.DeleteOptions{})
		}).Should(gomega.Succeed(), "delete pod after downgrade")
		ktesting.Eventually(tCtx, func(tCtx ktesting.TContext) *v1.Pod {
			pod, err := tCtx.Client().CoreV1().Pods(namespace.Name).Get(tCtx, pod.Name, metav1.GetOptions{})
