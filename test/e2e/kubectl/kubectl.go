...
var _ = SIGDescribe("Kubectl client", func() {
	...
	framework.KubeDescribe("Update Demo", func() {
		...
		BeforeEach(func() {
			...
			nautilus = substituteImageName(string(testfiles.ReadOrDie(filepath.Join(updateDemoRoot, "nautilus-rc.yaml.in"))))
			kitten = substituteImageName(string(testfiles.ReadOrDie(filepath.Join(updateDemoRoot, "kitten-rc.yaml.in"))))
		})
		/*
			Release : v1.9
	...
	framework.KubeDescribe("Guestbook application", func() {
		forEachGBFile := func(run func(s string)) {
			...
			for _, gbAppFile := range []string{
				...
				contents := substituteImageName(string(testfiles.ReadOrDie(filepath.Join(guestbookRoot, gbAppFile))))
				run(contents)
			}
		}
