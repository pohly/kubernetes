...
var _ = SIGDescribe("CronJob", func() {
	...
	*/
	framework.ConformanceIt("should support CronJob API operations", func(ctx context.Context) {
		...
		framework.ExpectNoError(err)
		gomega.Expect(clusterCJs.Items).To(gomega.HaveLen(1), "filtered list should have 1 items")
		...
		framework.ExpectNoError(err)
		gomega.Expect(patchedCronJob.Annotations["patched"]).To(gomega.Equal("true"), "patched object should have the applied annotation")
		...
		framework.ExpectNoError(err)
		gomega.Expect(updatedCronJob.Annotations["updated"]).To(gomega.Equal("true"), "updated object should have the applied annotation")
		...
		framework.ExpectNoError(err)
		gomega.Expect(string(createdCronJob.UID)).To(gomega.Equal(statusUID), fmt.Sprintf("createdCronJob.UID: %v expected to match statusUID: %v ", createdCronJob.UID, statusUID))
