...
var _ = SIGDescribe("Certificates API [Privileged:ClusterAdmin]", func() {
	...
	*/
	framework.ConformanceIt("should support CSR API operations", func(ctx context.Context) {
		...
		framework.ExpectNoError(err)
		gomega.Expect(patchedCSR.Annotations["patched"]).To(gomega.Equal("true"), "patched object should have the applied annotation")
		...
		framework.ExpectNoError(err)
		gomega.Expect(updatedCSR.Annotations["updated"]).To(gomega.Equal("true"), "updated object should have the applied annotation")
		...
		framework.ExpectNoError(err)
		gomega.Expect(patchedApproval.Status.Conditions).To(gomega.HaveLen(1), fmt.Sprintf("patched object should have the applied condition, got %#v", patchedApproval.Status.Conditions))
		gomega.Expect(string(patchedApproval.Status.Conditions[0].Type)).To(gomega.Equal("ApprovalPatch"), fmt.Sprintf("patched object should have the applied condition, got %#v", patchedApproval.Status.Conditions))
		gomega.Expect(patchedApproval.Annotations["patchedapproval"]).To(gomega.Equal("true"), "patched object should have the applied annotation")
		...
		framework.ExpectNoError(err)
		gomega.Expect(updatedApproval.Status.Conditions).To(gomega.HaveLen(2), fmt.Sprintf("updated object should have the applied condition, got %#v", updatedApproval.Status.Conditions))
		gomega.Expect(updatedApproval.Status.Conditions[1].Type).To(gomega.Equal(certificatesv1.CertificateApproved), fmt.Sprintf("updated object should have the approved condition, got %#v", updatedApproval.Status.Conditions))
		...
		framework.ExpectNoError(err)
		gomega.Expect(updatedStatus.Status.Conditions).To(gomega.HaveLen(len(statusToUpdate.Status.Conditions)), fmt.Sprintf("updated object should have the applied condition, got %#v", updatedStatus.Status.Conditions))
		gomega.Expect(string(updatedStatus.Status.Conditions[len(updatedStatus.Status.Conditions)-1].Type)).To(gomega.Equal("StatusUpdate"), fmt.Sprintf("updated object should have the approved condition, got %#v", updatedStatus.Status.Conditions))
