...
var _ = SIGDescribe("Container Runtime", func() {
	...
	ginkgo.Describe("blackbox test", func() {
		ginkgo.Context("when starting a container that exits", func() {
			...
			*/
			framework.ConformanceIt("should run with the expected status", framework.WithNodeConformance(), func(ctx context.Context) {
...
`
		...
		ginkgo.Context("on terminated container", func() {
			...
			*/
			framework.ConformanceIt("should report termination message if TerminationMessagePath is set as non-root user and at a non-default path", framework.WithNodeConformance(), func(ctx context.Context) {
				container := v1.Container{
					Image:                  framework.BusyBoxImage,
					Command:                []string{"/bin/sh", "-c"},
			...
			*/
			framework.ConformanceIt("should report termination message from log output if TerminationMessagePolicy FallbackToLogsOnError is set", framework.WithNodeConformance(), func(ctx context.Context) {
				container := v1.Container{
					Image:                    framework.BusyBoxImage,
					Command:                  []string{"/bin/sh", "-c"},
			...
			*/
			framework.ConformanceIt("should report termination message as empty when pod succeeds and TerminationMessagePolicy FallbackToLogsOnError is set", framework.WithNodeConformance(), func(ctx context.Context) {
				container := v1.Container{
					Image:                    framework.BusyBoxImage,
					Command:                  []string{"/bin/sh", "-c"},
			...
			*/
			framework.ConformanceIt("should report termination message from file when pod succeeds and TerminationMessagePolicy FallbackToLogsOnError is set", framework.WithNodeConformance(), func(ctx context.Context) {
				container := v1.Container{
					Image:                    framework.BusyBoxImage,
					Command:                  []string{"/bin/sh", "-c"},
