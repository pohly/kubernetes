...
var _ = SIGDescribe("RuntimeClass", func() {
	...
	*/
	framework.ConformanceIt(" should support RuntimeClasses API operations", func(ctx context.Context) {
		...
		framework.ExpectNoError(err)
		gomega.Expect(patchedRC.Annotations["patched"]).To(gomega.Equal("true"), "patched object should have the applied annotation")
		...
		framework.ExpectNoError(err)
		gomega.Expect(updatedRC.Annotations["updated"]).To(gomega.Equal("true"), "updated object should have the applied annotation")
