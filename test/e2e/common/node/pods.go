...
var _ = SIGDescribe("Pods", func() {
	...
	*/
	framework.ConformanceIt("should run through the lifecycle of Pods and PodStatus", func(ctx context.Context) {
		...
		gomega.Expect(pod.ObjectMeta.Labels["test-pod"]).To(gomega.Equal("patched"), "failed to patch Pod - missing label")
	...
	*/
	framework.ConformanceIt("should patch a pod status", func(ctx context.Context) {
		...
		gomega.Expect(pStatus.Status.Message).To(gomega.Equal("Patched by e2e test"), fmt.Sprintf("Status.Message for %q was %q but expected it to be \"Patched by e2e test\"", podName, pStatus.Status.Message))
		gomega.Expect(pStatus.Status.Reason).To(gomega.Equal("E2E"), fmt.Sprintf("Status.Reason for %q was %q but expected it to be \"E2E\"", podName, pStatus.Status.Reason))
