...
var _ = SIGDescribe("ConfigMap", func() {
	...
	*/
	framework.ConformanceIt("should be consumable via environment variable", framework.WithNodeConformance(), func(ctx context.Context) {
	...
	*/
	framework.ConformanceIt("should be consumable via the environment", framework.WithNodeConformance(), func(ctx context.Context) {
		name := "configmap-test-" + string(uuid.NewUUID())
		configMap := newConfigMap(f, name)
		ginkgo.By(fmt.Sprintf("Creating configMap %v/%v", f.Namespace.Name, configMap.Name))
		var err error
